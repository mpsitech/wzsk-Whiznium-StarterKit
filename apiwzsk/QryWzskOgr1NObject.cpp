/**
	...
	* API code for job QryWzskOgr1NObject (implementation)
...
QryWzskOgr1NObject::StatApp::StatApp(
			const uint firstcol
...
bool QryWzskOgr1NObject::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryWzskOgr1NObject");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppQryWzskOgr1NObject";
...
set<uint> QryWzskOgr1NObject::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryWzskOgr1NObject::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryWzskOgr1NObject::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryWzskOgr1NObject");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrQryWzskOgr1NObject";
