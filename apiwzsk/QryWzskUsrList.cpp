...
QryWzskUsrList::StatApp::StatApp(
			const uint firstcol
...
bool QryWzskUsrList::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryWzskUsrList");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppQryWzskUsrList";
...
set<uint> QryWzskUsrList::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryWzskUsrList::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryWzskUsrList::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryWzskUsrList");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrQryWzskUsrList";
