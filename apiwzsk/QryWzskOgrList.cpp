...
QryWzskOgrList::StatApp::StatApp(
			const uint firstcol
...
bool QryWzskOgrList::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryWzskOgrList");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppQryWzskOgrList";
...
set<uint> QryWzskOgrList::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryWzskOgrList::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryWzskOgrList::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryWzskOgrList");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrQryWzskOgrList";
