/**
	* \file CrdWzskSes.cpp
	* API code for job CrdWzskSes (implementation)
...
bool CrdWzskSes::ContInf::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "ContInfWzskSes");
	else
	...
	if (basefound) {
		...
		if (extractStringAttrUclc(docctx, basexpath, itemtag, "Ci", "sref", "MtxCrdSes", MtxCrdSes)) add(MTXCRDSES);
	};
...
bool CrdWzskSes::StatApp::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppWzskSes");
	else
		basefound = checkXPath(docctx, basexpath);
...
set<uint> CrdWzskSes::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> CrdWzskSes::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool CrdWzskSes::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrWzskSes");
	else
		basefound = checkXPath(docctx, basexpath);
...
bool CrdWzskSes::Tag::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagWzskSes");
	else
		basefound = checkXPath(docctx, basexpath);
...
CrdWzskSes::DpchAppDo::DpchAppDo(
			...
			DpchAppWzsk(VecWzskVDpch::DPCHAPPWZSKSESDO, scrJref)
		{
...
void CrdWzskSes::DpchAppDo::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppWzskSesDo");
...
CrdWzskSes::DpchEngData::DpchEngData() :
			DpchEngWzsk(VecWzskVDpch::DPCHENGWZSKSESDATA)
		{
...
void CrdWzskSes::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngWzskSesData");
	else
		basefound = checkXPath(docctx, basexpath);
