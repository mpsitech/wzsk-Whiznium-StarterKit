/**
	...
	* API code for job PnlWzskObjList (implementation)
...
bool PnlWzskObjList::ContInf::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "ContInfWzskObjList");
	else
		basefound = checkXPath(docctx, basexpath);
...
bool PnlWzskObjList::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrWzskObjList");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlWzskObjList::Tag::Tag(
	...
	mask = {CPT, TXTRECORD1, TXTRECORD2, TRS, TXTSHOWING1, TXTSHOWING2, TCOTIT, TCOOGR};
};

bool PnlWzskObjList::Tag::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagWzskObjList");
	else
		basefound = checkXPath(docctx, basexpath);
	...
	if (basefound) {
		...
		if (extractStringAttrUclc(docctx, basexpath, itemtag, "Ti", "sref", "TcoOgr", TcoOgr)) add(TCOOGR);
	};
...
PnlWzskObjList::DpchAppData::DpchAppData(
			...
			DpchAppWzsk(VecWzskVDpch::DPCHAPPWZSKOBJLISTDATA, scrJref)
		{
...
void PnlWzskObjList::DpchAppData::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppWzskObjListData");
...
PnlWzskObjList::DpchAppDo::DpchAppDo(
			...
			DpchAppWzsk(VecWzskVDpch::DPCHAPPWZSKOBJLISTDO, scrJref)
		{
...
void PnlWzskObjList::DpchAppDo::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppWzskObjListDo");
	xmlTextWriterWriteAttribute(wr, BAD_CAST "xmlns", BAD_CAST "http://www.mpsitech.com/wzsk");
		if (has(SCRJREF)) writeString(wr, "scrJref", scrJref);
...
PnlWzskObjList::DpchEngData::DpchEngData() :
			DpchEngWzsk(VecWzskVDpch::DPCHENGWZSKOBJLISTDATA)
		{
...
void PnlWzskObjList::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngWzskObjListData");
	else
		basefound = checkXPath(docctx, basexpath);
