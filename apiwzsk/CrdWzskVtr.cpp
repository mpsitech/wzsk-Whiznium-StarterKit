/**
	* \file CrdWzskVtr.cpp
	* API code for job CrdWzskVtr (implementation)
...
bool CrdWzskVtr::ContInf::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "ContInfWzskVtr");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "ContitemInfWzskVtr";
...
CrdWzskVtr::StatApp::StatApp(
			...
			, initdoneMonitor(initdoneMonitor)
			, initdoneDebug(initdoneDebug)
			, initdoneHeadbar(initdoneHeadbar)
		{
	mask = {IXWZSKVREQITMODE, LATENCY, SHORTMENU, WIDTHMENU, INITDONECONFIG, INITDONEMONITOR, INITDONEDEBUG, INITDONEHEADBAR};
};

bool CrdWzskVtr::StatApp::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppWzskVtr");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppWzskVtr";
	...
	if (basefound) {
		...
		if (extractBoolAttrUclc(docctx, basexpath, itemtag, "Si", "sref", "initdoneMonitor", initdoneMonitor)) add(INITDONEMONITOR);
...
set<uint> CrdWzskVtr::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
	...
	if (initdoneMonitor == comp->initdoneMonitor) insert(items, INITDONEMONITOR);
...
set<uint> CrdWzskVtr::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
	...
	diffitems = {IXWZSKVREQITMODE, LATENCY, SHORTMENU, WIDTHMENU, INITDONECONFIG, INITDONEMONITOR, INITDONEDEBUG, INITDONEHEADBAR};
	for (auto it = commitems.begin(); it != commitems.end(); it++) diffitems.erase(*it);
...
CrdWzskVtr::StatShr::StatShr(
			...
			, const string& scrJrefMonitor
			, const string& scrJrefDebug
			, const string& scrJrefHeadbar
		) :
			Block()
			, scrJrefConfig(scrJrefConfig)
			, scrJrefMonitor(scrJrefMonitor)
			, scrJrefDebug(scrJrefDebug)
			, scrJrefHeadbar(scrJrefHeadbar)
		{
	mask = {SCRJREFCONFIG, SCRJREFMONITOR, SCRJREFDEBUG, SCRJREFHEADBAR};
};

bool CrdWzskVtr::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrWzskVtr");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrWzskVtr";

	if (basefound) {
		if (extractStringAttrUclc(docctx, basexpath, itemtag, "Si", "sref", "scrJrefConfig", scrJrefConfig)) add(SCRJREFCONFIG);
		if (extractStringAttrUclc(docctx, basexpath, itemtag, "Si", "sref", "scrJrefMonitor", scrJrefMonitor)) add(SCRJREFMONITOR);
...
set<uint> CrdWzskVtr::StatShr::comm(
	...
	if (scrJrefMonitor == comp->scrJrefMonitor) insert(items, SCRJREFMONITOR);
...
set<uint> CrdWzskVtr::StatShr::diff(
	...
	diffitems = {SCRJREFCONFIG, SCRJREFMONITOR, SCRJREFDEBUG, SCRJREFHEADBAR};
	for (auto it = commitems.begin(); it != commitems.end(); it++) diffitems.erase(*it);
...
bool CrdWzskVtr::Tag::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagWzskVtr");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "TagitemWzskVtr";
...
CrdWzskVtr::DpchAppDo::DpchAppDo(
			...
			DpchAppWzsk(VecWzskVDpch::DPCHAPPWZSKVTRDO, scrJref)
			, ixVDo(ixVDo)
		{
...
void CrdWzskVtr::DpchAppDo::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppWzskVtrDo");
...
CrdWzskVtr::DpchEngData::DpchEngData() :
			DpchEngWzsk(VecWzskVDpch::DPCHENGWZSKVTRDATA)
		{
...
void CrdWzskVtr::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngWzskVtrData");
	else
		basefound = checkXPath(docctx, basexpath);
