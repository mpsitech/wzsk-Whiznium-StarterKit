...
QryWzskUsr1NSession::StatApp::StatApp(
			const uint firstcol
...
bool QryWzskUsr1NSession::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryWzskUsr1NSession");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppQryWzskUsr1NSession";
...
set<uint> QryWzskUsr1NSession::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryWzskUsr1NSession::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryWzskUsr1NSession::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryWzskUsr1NSession");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrQryWzskUsr1NSession";
