/**
	* \file PnlWzskOgrHeadbar.cpp
	* API code for job PnlWzskOgrHeadbar (implementation)
...
PnlWzskOgrHeadbar::StgInf::StgInf(
...
bool PnlWzskOgrHeadbar::StgInf::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StgInfWzskOgrHeadbar");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StgitemInfWzskOgrHeadbar";
...
set<uint> PnlWzskOgrHeadbar::StgInf::comm(
			const StgInf* comp
		) {
	set<uint> items;
...
set<uint> PnlWzskOgrHeadbar::StgInf::diff(
			const StgInf* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool PnlWzskOgrHeadbar::Tag::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagWzskOgrHeadbar");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "TagitemWzskOgrHeadbar";
...
PnlWzskOgrHeadbar::DpchEngData::DpchEngData() :
			DpchEngWzsk(VecWzskVDpch::DPCHENGWZSKOGRHEADBARDATA)
		{
};
...
void PnlWzskOgrHeadbar::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngWzskOgrHeadbarData");
	else
		basefound = checkXPath(docctx, basexpath);
