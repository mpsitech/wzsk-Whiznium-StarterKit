/**
	* \file PnlWzskUsgHeadbar.cpp
	* API code for job PnlWzskUsgHeadbar (implementation)
...
PnlWzskUsgHeadbar::StgInf::StgInf(
...
bool PnlWzskUsgHeadbar::StgInf::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StgInfWzskUsgHeadbar");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StgitemInfWzskUsgHeadbar";
...
set<uint> PnlWzskUsgHeadbar::StgInf::comm(
			const StgInf* comp
		) {
	set<uint> items;
...
set<uint> PnlWzskUsgHeadbar::StgInf::diff(
			const StgInf* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool PnlWzskUsgHeadbar::Tag::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagWzskUsgHeadbar");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "TagitemWzskUsgHeadbar";
...
PnlWzskUsgHeadbar::DpchEngData::DpchEngData() :
			DpchEngWzsk(VecWzskVDpch::DPCHENGWZSKUSGHEADBARDATA)
		{
};
...
void PnlWzskUsgHeadbar::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngWzskUsgHeadbarData");
	else
		basefound = checkXPath(docctx, basexpath);
