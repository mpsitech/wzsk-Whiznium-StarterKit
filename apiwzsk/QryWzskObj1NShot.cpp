/**
	...
	* API code for job QryWzskObj1NShot (implementation)
...
QryWzskObj1NShot::StatApp::StatApp(
			const uint firstcol
...
bool QryWzskObj1NShot::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryWzskObj1NShot");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppQryWzskObj1NShot";
...
set<uint> QryWzskObj1NShot::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryWzskObj1NShot::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryWzskObj1NShot::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryWzskObj1NShot");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrQryWzskObj1NShot";
