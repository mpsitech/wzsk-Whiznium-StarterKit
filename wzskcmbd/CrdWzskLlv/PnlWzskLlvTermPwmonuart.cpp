/**
	* \file PnlWzskLlvTermPwmonuart.cpp
	* job handler for job PnlWzskLlvTermPwmonuart (implementation)
...
#include "PnlWzskLlvTermPwmonuart_blks.cpp"
...
/******************************************************************************
 class PnlWzskLlvTermPwmonuart
 ******************************************************************************/

PnlWzskLlvTermPwmonuart::PnlWzskLlvTermPwmonuart(
			XchgWzsk* xchg
			, DbsWzsk* dbswzsk
			, const ubigint jrefSup
			, const uint ixWzskVLocale
		) :
			JobWzsk(xchg, VecWzskVJob::PNLWZSKLLVTERMPWMONUART, jrefSup, ixWzskVLocale)
		{
	...
	UntPmndUdua& hw = srcpwmonuart->shrdat.hw;
	...
	xchg->addClstn(VecWzskVCall::CALLWZSKCLAIMCHG, jref, Clstn::VecVJobmask::SPEC, srcpwmonuart->jref, false, Arg(), 0, Clstn::VecVJactype::WEAK);
...
DpchEngWzsk* PnlWzskLlvTermPwmonuart::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWzskLlvTermPwmonuart::refresh(
			DbsWzsk* dbswzsk
			, set<uint>& moditems
			, const bool unmute
		) {
	...
	xchg->getCsjobClaim(srcpwmonuart, takenNotAvailable, fulfilled);

	// continf 
	ContInf oldContinf(continf);

	continf.ButClaimOn = fulfilled;
	continf.TxtCst = srcpwmonuart->getSquawk(dbswzsk);
...
void PnlWzskLlvTermPwmonuart::handleRequest(
			DbsWzsk* dbswzsk
			, ReqWzsk* req
		) {
	if (req->ixVBasetype == ReqWzsk::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWzskVDpch == VecWzskVDpch::DPCHAPPWZSKLLVTERMPWMONUARTDATA) {
		...
		} else if (req->dpchapp->ixWzskVDpch == VecWzskVDpch::DPCHAPPWZSKLLVTERMPWMONUARTDO) {
...
void PnlWzskLlvTermPwmonuart::handleDpchAppWzskInit(
			DbsWzsk* dbswzsk
...
void PnlWzskLlvTermPwmonuart::handleDpchAppDataContiac(
			DbsWzsk* dbswzsk
			, ContIac* _contiac
			, DpchEngWzsk** dpcheng
		) {
...
void PnlWzskLlvTermPwmonuart::handleDpchAppDoButRegularizeClick(
			DbsWzsk* dbswzsk
			, DpchEngWzsk** dpcheng
		) {
...
void PnlWzskLlvTermPwmonuart::handleDpchAppDoButMinimizeClick(
			DbsWzsk* dbswzsk
			, DpchEngWzsk** dpcheng
		) {
...
void PnlWzskLlvTermPwmonuart::handleDpchAppDoButClaimClick(
			DbsWzsk* dbswzsk
			, DpchEngWzsk** dpcheng
		) {
	...
	if (statshr.ButClaimActive) {
		...
		if (!continf.ButClaimOn) xchg->addCsjobClaim(dbswzsk, srcpwmonuart, new Claim());
		else xchg->removeCsjobClaim(dbswzsk, srcpwmonuart);
...
void PnlWzskLlvTermPwmonuart::handleDpchAppDoButApdClick(
			DbsWzsk* dbswzsk
			, DpchEngWzsk** dpcheng
		) {
...
void PnlWzskLlvTermPwmonuart::handleDpchAppDoButSmtClick(
			DbsWzsk* dbswzsk
			, DpchEngWzsk** dpcheng
		) {
...
void PnlWzskLlvTermPwmonuart::handleCall(
			DbsWzsk* dbswzsk
			, Call* call
		) {
	if ((call->ixVCall == VecWzskVCall::CALLWZSKCLAIMCHG) && (call->jref == srcpwmonuart->jref)) {
		call->abort = handleCallWzskClaimChgFromSrcpwmonuart(dbswzsk);
	};
};

bool PnlWzskLlvTermPwmonuart::handleCallWzskClaimChgFromSrcpwmonuart(
			DbsWzsk* dbswzsk
		) {
	bool retval = false;
	// IP handleCallWzskClaimChgFromSrcpwmonuart --- IBEGIN
	set<uint> moditems;

	refresh(dbswzsk, moditems);
	if (!moditems.empty()) xchg->submitDpch(getNewDpchEng(moditems));
	// IP handleCallWzskClaimChgFromSrcpwmonuart --- IEND
	return retval;
};
