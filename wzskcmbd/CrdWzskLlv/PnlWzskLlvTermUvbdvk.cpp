/**
	...
	* job handler for job PnlWzskLlvTermUvbdvk (implementation)
...
#include "PnlWzskLlvTermUvbdvk_blks.cpp"
...
/******************************************************************************
 class PnlWzskLlvTermUvbdvk
 ******************************************************************************/

PnlWzskLlvTermUvbdvk::PnlWzskLlvTermUvbdvk(
			XchgWzsk* xchg
			, DbsWzsk* dbswzsk
			, const ubigint jrefSup
			, const uint ixWzskVLocale
		) :
			JobWzsk(xchg, VecWzskVJob::PNLWZSKLLVTERMUVBDVK, jrefSup, ixWzskVLocale)
		{
	...
	UntWskdUbdk& hw = srcuvbdvk->shrdat.hw;
	...
	xchg->addClstn(VecWzskVCall::CALLWZSKCLAIMCHG, jref, Clstn::VecVJobmask::SPEC, srcuvbdvk->jref, false, Arg(), 0, Clstn::VecVJactype::WEAK);
...
DpchEngWzsk* PnlWzskLlvTermUvbdvk::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWzskLlvTermUvbdvk::refresh(
			DbsWzsk* dbswzsk
			, set<uint>& moditems
			, const bool unmute
		) {
	...
	xchg->getCsjobClaim(srcuvbdvk, takenNotAvailable, fulfilled);

	// continf 
	ContInf oldContinf(continf);

	continf.ButClaimOn = fulfilled;
	continf.TxtCst = srcuvbdvk->getSquawk(dbswzsk);
...
void PnlWzskLlvTermUvbdvk::handleRequest(
			DbsWzsk* dbswzsk
			, ReqWzsk* req
		) {
	if (req->ixVBasetype == ReqWzsk::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWzskVDpch == VecWzskVDpch::DPCHAPPWZSKLLVTERMUVBDVKDATA) {
		...
		} else if (req->dpchapp->ixWzskVDpch == VecWzskVDpch::DPCHAPPWZSKLLVTERMUVBDVKDO) {
...
void PnlWzskLlvTermUvbdvk::handleDpchAppWzskInit(
			DbsWzsk* dbswzsk
...
void PnlWzskLlvTermUvbdvk::handleDpchAppDataContiac(
			DbsWzsk* dbswzsk
			, ContIac* _contiac
			, DpchEngWzsk** dpcheng
		) {
...
void PnlWzskLlvTermUvbdvk::handleDpchAppDoButRegularizeClick(
			DbsWzsk* dbswzsk
			, DpchEngWzsk** dpcheng
		) {
...
void PnlWzskLlvTermUvbdvk::handleDpchAppDoButMinimizeClick(
			DbsWzsk* dbswzsk
			, DpchEngWzsk** dpcheng
		) {
...
void PnlWzskLlvTermUvbdvk::handleDpchAppDoButClaimClick(
			DbsWzsk* dbswzsk
			, DpchEngWzsk** dpcheng
		) {
	...
	if (statshr.ButClaimActive) {
		...
		if (!continf.ButClaimOn) xchg->addCsjobClaim(dbswzsk, srcuvbdvk, new JobWzskSrcUvbdvk::Claim(true, true, false, false));
		else xchg->removeCsjobClaim(dbswzsk, srcuvbdvk);
...
void PnlWzskLlvTermUvbdvk::handleDpchAppDoButApdClick(
			DbsWzsk* dbswzsk
			, DpchEngWzsk** dpcheng
		) {
...
void PnlWzskLlvTermUvbdvk::handleDpchAppDoButSmtClick(
			DbsWzsk* dbswzsk
			, DpchEngWzsk** dpcheng
		) {
...
void PnlWzskLlvTermUvbdvk::handleCall(
			DbsWzsk* dbswzsk
			, Call* call
		) {
	if ((call->ixVCall == VecWzskVCall::CALLWZSKCLAIMCHG) && (call->jref == srcuvbdvk->jref)) {
		call->abort = handleCallWzskClaimChgFromSrcuvbdvk(dbswzsk);
	};
};

bool PnlWzskLlvTermUvbdvk::handleCallWzskClaimChgFromSrcuvbdvk(
			DbsWzsk* dbswzsk
		) {
	bool retval = false;
	// IP handleCallWzskClaimChgFromSrcuvbdvk --- IBEGIN
	set<uint> moditems;

	refresh(dbswzsk, moditems);
	if (!moditems.empty()) xchg->submitDpch(getNewDpchEng(moditems));
	// IP handleCallWzskClaimChgFromSrcuvbdvk --- IEND
	return retval;
};
