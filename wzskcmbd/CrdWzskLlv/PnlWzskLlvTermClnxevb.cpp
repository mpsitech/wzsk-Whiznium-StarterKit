...
PnlWzskLlvTermClnxevb::PnlWzskLlvTermClnxevb(
	...
	UntWskdCleb& hw = srcclnxevb->shrdat.hw;
...
void PnlWzskLlvTermClnxevb::refresh(
	...
	xchg->getCsjobClaim(srcclnxevb, takenNotAvailable, fulfilled);

	// continf 
	ContInf oldContinf(continf);

	continf.ButClaimOn = fulfilled;
	continf.TxtCst = srcclnxevb->getSquawk(dbswzsk);
...
void PnlWzskLlvTermClnxevb::handleDpchAppDoButClaimClick(
	...
	if (statshr.ButClaimActive) {
		...
		if (!continf.ButClaimOn) xchg->addCsjobClaim(dbswzsk, srcclnxevb, new JobWzskSrcClnxevb::Claim(true, true, false, false));
		else xchg->removeCsjobClaim(dbswzsk, srcclnxevb);
...
bool PnlWzskLlvTermClnxevb::handleCallWzskClaimChgFromSrcclnxevb(
	...
	bool retval = false;
	// IP handleCallWzskClaimChgFromSrcclnxevb --- IBEGIN
	set<uint> moditems;

	refresh(dbswzsk, moditems);
	if (!moditems.empty()) xchg->submitDpch(getNewDpchEng(moditems));
	// IP handleCallWzskClaimChgFromSrcclnxevb --- IEND
	return retval;
};
