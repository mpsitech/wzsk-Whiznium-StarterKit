...
PnlWzskLiv2DView::PnlWzskLiv2DView(
	...
	feedFPupPvm[VecWzskVPvwmode::BINREDDOM]->Avail = ((xchg->stgwzskglobal.ixWzskVTarget == VecWzskVTarget::APALIS) || (xchg->stgwzskglobal.ixWzskVTarget == VecWzskVTarget::WS));
...
void PnlWzskLiv2DView::refresh(
	...
	contiac.SldCwd = acqptcloud->shrdat.dWork;
	...
	statshr.UpdLloAvail = ((xchg->stgwzskglobal.ixWzskVTarget != VecWzskVTarget::APALIS) && (xchg->stgwzskglobal.ixWzskVTarget != VecWzskVTarget::WS)) || iprtrace->stg.v4l2RdNotDelta;
	statshr.UpdLuoAvail = ((xchg->stgwzskglobal.ixWzskVTarget != VecWzskVTarget::APALIS) && (xchg->stgwzskglobal.ixWzskVTarget != VecWzskVTarget::WS)) || iprtrace->stg.v4l2RdNotDelta;

	statshr.UpdLmdAvail = ((xchg->stgwzskglobal.ixWzskVTarget == VecWzskVTarget::APALIS) || (xchg->stgwzskglobal.ixWzskVTarget == VecWzskVTarget::WS)) && !iprtrace->stg.v4l2RdNotDelta;
	// IP refresh --- REND
...
void PnlWzskLiv2DView::handleDpchAppDataContiac(
	...
	if (has(diffitems, ContIac::SLDCWD)) acqptcloud->setDWork(dbswzsk, _contiac->SldCwd);
...
void PnlWzskLiv2DView::handleCall(
	...
	if ((call->ixVCall == VecWzskVCall::CALLWZSKSGECHG) && (call->jref == actservo->jref)) {
		...
		call->abort = handleCallWzskShrdatChgFromAcqptcloud(dbswzsk, call->argInv.ix, call->argInv.sref);
...
bool PnlWzskLiv2DView::handleCallWzskShrdatChgFromAcqptcloud(
			DbsWzsk* dbswzsk
			, const uint ixInv
			, const string& srefInv
		) {
	bool retval = false;
	// IP handleCallWzskShrdatChgFromAcqptcloud --- IBEGIN
	set<uint> moditems;

	refresh(dbswzsk, moditems);
	if (!moditems.empty()) xchg->submitDpch(getNewDpchEng(moditems));
	// IP handleCallWzskShrdatChgFromAcqptcloud --- IEND
	return retval;
};
