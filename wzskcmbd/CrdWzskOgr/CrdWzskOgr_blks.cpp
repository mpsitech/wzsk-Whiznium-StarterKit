/**
	* \file CrdWzskOgr_blks.cpp
	* job handler for job CrdWzskOgr (implementation of blocks)
...
void CrdWzskOgr::VecVSge::fillFeed(
			Feed& feed
		) {
	feed.clear();
...
void CrdWzskOgr::ContInf::writeJSON(
			Json::Value& sup
			, string difftag
		) {
	if (difftag.length() == 0) difftag = "ContInfWzskOgr";

	Json::Value& me = sup[difftag] = Json::Value(Json::objectValue);

	me["numFSge"] = numFSge;
	me["MrlAppHlp"] = MrlAppHlp;
	me["MtxCrdOgr"] = MtxCrdOgr;
};

void CrdWzskOgr::ContInf::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	...
	else itemtag = "ContitemInfWzskOgr";

	xmlTextWriterStartElement(wr, BAD_CAST difftag.c_str());
		writeUintAttr(wr, itemtag, "sref", "numFSge", numFSge);
		writeStringAttr(wr, itemtag, "sref", "MrlAppHlp", MrlAppHlp);
		writeStringAttr(wr, itemtag, "sref", "MtxCrdOgr", MtxCrdOgr);
...
void CrdWzskOgr::StatApp::writeJSON(
			Json::Value& sup
			, string difftag
	...
	if (difftag.length() == 0) difftag = "StatAppWzskOgr";
...
void CrdWzskOgr::StatApp::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
			, const uint ixWzskVReqitmode
	...
	else itemtag = "StatitemAppWzskOgr";
...
void CrdWzskOgr::StatShr::writeJSON(
			Json::Value& sup
			, string difftag
		) {
	if (difftag.length() == 0) difftag = "StatShrWzskOgr";
...
void CrdWzskOgr::StatShr::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	...
	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemShrWzskOgr";
...
void CrdWzskOgr::Tag::writeJSON(
			const uint ixWzskVLocale
			, Json::Value& sup
			, string difftag
		) {
	if (difftag.length() == 0) difftag = "TagWzskOgr";
...
CrdWzskOgr::DpchAppDo::DpchAppDo() :
			DpchAppWzsk(VecWzskVDpch::DPCHAPPWZSKOGRDO)
		{
	ixVDo = 0;
};
...
void CrdWzskOgr::DpchAppDo::readJSON(
	...
	const Json::Value& me = [&]{if (!addbasetag) return sup; return sup["DpchAppWzskOgrDo"];}();
...
void CrdWzskOgr::DpchAppDo::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppWzskOgrDo");
	else
...
CrdWzskOgr::DpchEngData::DpchEngData(
			...
			DpchEngWzsk(VecWzskVDpch::DPCHENGWZSKOGRDATA, jref)
		{
...
void CrdWzskOgr::DpchEngData::merge(
			DpchEngWzsk* dpcheng
		) {
...
void CrdWzskOgr::DpchEngData::writeJSON(
			const uint ixWzskVLocale
			, Json::Value& sup
		) {
	Json::Value& me = sup["DpchEngWzskOgrData"] = Json::Value(Json::objectValue);
...
void CrdWzskOgr::DpchEngData::writeXML(
			const uint ixWzskVLocale
			, xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchEngWzskOgrData");
