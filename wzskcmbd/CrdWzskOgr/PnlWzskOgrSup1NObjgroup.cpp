...
#include "PnlWzskOgrSup1NObjgroup_blks.cpp"
#include "PnlWzskOgrSup1NObjgroup_evals.cpp"
...
/******************************************************************************
 class PnlWzskOgrSup1NObjgroup
 ******************************************************************************/

PnlWzskOgrSup1NObjgroup::PnlWzskOgrSup1NObjgroup(
			XchgWzsk* xchg
			, DbsWzsk* dbswzsk
			, const ubigint jrefSup
			, const uint ixWzskVLocale
		) :
			JobWzsk(xchg, VecWzskVJob::PNLWZSKOGRSUP1NOBJGROUP, jrefSup, ixWzskVLocale)
		{
	...
	qry = new QryWzskOgrSup1NObjgroup(xchg, dbswzsk, jref, ixWzskVLocale);
...
DpchEngWzsk* PnlWzskOgrSup1NObjgroup::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWzskOgrSup1NObjgroup::refresh(
			DbsWzsk* dbswzsk
			, set<uint>& moditems
		) {
...
void PnlWzskOgrSup1NObjgroup::updatePreset(
			DbsWzsk* dbswzsk
...
void PnlWzskOgrSup1NObjgroup::handleRequest(
			DbsWzsk* dbswzsk
			, ReqWzsk* req
		) {
	if (req->ixVBasetype == ReqWzsk::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWzskVDpch == VecWzskVDpch::DPCHAPPWZSKOGRSUP1NOBJGROUPDATA) {
		...
		} else if (req->dpchapp->ixWzskVDpch == VecWzskVDpch::DPCHAPPWZSKOGRSUP1NOBJGROUPDO) {
...
void PnlWzskOgrSup1NObjgroup::handleDpchAppWzskInit(
			DbsWzsk* dbswzsk
...
void PnlWzskOgrSup1NObjgroup::handleDpchAppDataStgiac(
			DbsWzsk* dbswzsk
			, StgIac* _stgiac
			, DpchEngWzsk** dpcheng
		) {
...
void PnlWzskOgrSup1NObjgroup::handleDpchAppDataStgiacqry(
			DbsWzsk* dbswzsk
			, QryWzskOgrSup1NObjgroup::StgIac* _stgiacqry
			, DpchEngWzsk** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WzskQOgrSup1NObjgroup* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWzskOgrSup1NObjgroup::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWzskOgrSup1NObjgroup::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswzsk);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWzskOgrSup1NObjgroup::StgIac::JNUM)) {
			if (!recSelNew) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

			recOgr = WzskMObjgroup();

			if (recSelNew) {
				if (dbswzsk->tblwzskmobjgroup->loadRecByRef(recSelNew->ref, &_recOgr)) {
					recOgr = *_recOgr;
					delete _recOgr;
				};
			};
...
void PnlWzskOgrSup1NObjgroup::handleDpchAppDoButViewClick(
			DbsWzsk* dbswzsk
			, DpchEngWzsk** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecWzskVPreset::PREWZSKIXCRDACCOGR, jref)) {
			sref = "CrdWzskOgr";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswzsk, VecWzskVCall::CALLWZSKCRDOPEN, jref, 0, 0, sref, recOgr.ref, jrefNew);
		};
...
void PnlWzskOgrSup1NObjgroup::handleDpchAppDoButRefreshClick(
			DbsWzsk* dbswzsk
			, DpchEngWzsk** dpcheng
		) {
	set<uint> moditems;
...
void PnlWzskOgrSup1NObjgroup::handleCall(
			DbsWzsk* dbswzsk
			, Call* call
		) {
...
bool PnlWzskOgrSup1NObjgroup::handleCallWzskStatChg(
			DbsWzsk* dbswzsk
			, const ubigint jrefTrig
		) {
	bool retval = false;
