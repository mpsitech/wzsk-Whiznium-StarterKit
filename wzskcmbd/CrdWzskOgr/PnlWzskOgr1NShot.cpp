/**
	* \file PnlWzskOgr1NShot.cpp
	* job handler for job PnlWzskOgr1NShot (implementation)
...
#include "PnlWzskOgr1NShot_blks.cpp"
#include "PnlWzskOgr1NShot_evals.cpp"
...
/******************************************************************************
 class PnlWzskOgr1NShot
 ******************************************************************************/

PnlWzskOgr1NShot::PnlWzskOgr1NShot(
			XchgWzsk* xchg
			, DbsWzsk* dbswzsk
			, const ubigint jrefSup
			, const uint ixWzskVLocale
		) :
			JobWzsk(xchg, VecWzskVJob::PNLWZSKOGR1NSHOT, jrefSup, ixWzskVLocale)
		{
	...
	qry = new QryWzskOgr1NShot(xchg, dbswzsk, jref, ixWzskVLocale);
...
DpchEngWzsk* PnlWzskOgr1NShot::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWzskOgr1NShot::updatePreset(
			DbsWzsk* dbswzsk
...
void PnlWzskOgr1NShot::handleRequest(
			DbsWzsk* dbswzsk
			, ReqWzsk* req
		) {
	if (req->ixVBasetype == ReqWzsk::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWzskVDpch == VecWzskVDpch::DPCHAPPWZSKOGR1NSHOTDATA) {
		...
		} else if (req->dpchapp->ixWzskVDpch == VecWzskVDpch::DPCHAPPWZSKOGR1NSHOTDO) {
...
void PnlWzskOgr1NShot::handleDpchAppWzskInit(
			DbsWzsk* dbswzsk
...
void PnlWzskOgr1NShot::handleDpchAppDataStgiac(
			DbsWzsk* dbswzsk
			, StgIac* _stgiac
			, DpchEngWzsk** dpcheng
		) {
...
void PnlWzskOgr1NShot::handleDpchAppDataStgiacqry(
			DbsWzsk* dbswzsk
			, QryWzskOgr1NShot::StgIac* _stgiacqry
			, DpchEngWzsk** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWzskOgr1NShot::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWzskOgr1NShot::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswzsk);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWzskOgr1NShot::StgIac::JNUM)) {
...
void PnlWzskOgr1NShot::handleDpchAppDoButViewClick(
			DbsWzsk* dbswzsk
			, DpchEngWzsk** dpcheng
		) {
	ubigint jrefNew = 0;
...
void PnlWzskOgr1NShot::handleDpchAppDoButNewClick(
			DbsWzsk* dbswzsk
			, DpchEngWzsk** dpcheng
		) {
	ubigint jrefNew = 0;
...
void PnlWzskOgr1NShot::handleDpchAppDoButDeleteClick(
			DbsWzsk* dbswzsk
			, DpchEngWzsk** dpcheng
		) {
...
void PnlWzskOgr1NShot::handleDpchAppDoButRefreshClick(
			DbsWzsk* dbswzsk
			, DpchEngWzsk** dpcheng
		) {
	set<uint> moditems;
...
void PnlWzskOgr1NShot::handleCall(
			DbsWzsk* dbswzsk
			, Call* call
		) {
...
bool PnlWzskOgr1NShot::handleCallWzskStatChg(
			DbsWzsk* dbswzsk
			, const ubigint jrefTrig
		) {
	bool retval = false;
