/**
	* \file PnlWzskObjList.cpp
	* job handler for job PnlWzskObjList (implementation)
...
#include "PnlWzskObjList_blks.cpp"
#include "PnlWzskObjList_evals.cpp"
...
/******************************************************************************
 class PnlWzskObjList
 ******************************************************************************/

PnlWzskObjList::PnlWzskObjList(
			XchgWzsk* xchg
			, DbsWzsk* dbswzsk
			, const ubigint jrefSup
			, const uint ixWzskVLocale
		) :
			JobWzsk(xchg, VecWzskVJob::PNLWZSKOBJLIST, jrefSup, ixWzskVLocale)
		{
	...
	QryWzskObjList::VecVOrd::fillFeed(feedFTos);

	qry = NULL;

	// IP constructor.cust1 --- INSERT

	xchg->addRefPreset(VecWzskVPreset::PREWZSKREFSEL, jref, xchg->getRefPreset(VecWzskVPreset::PREWZSKREFOBJ, jref));
	xchg->addIxPreset(VecWzskVPreset::PREWZSKIXORD, jref, QryWzskObjList::VecVOrd::TIT);

	qry = new QryWzskObjList(xchg, dbswzsk, jref, ixWzskVLocale);

	// IP constructor.cust2 --- INSERT

	set<uint> moditems;
	statshr.ixWzskVExpstate = ((xchg->getRefPreset(VecWzskVPreset::PREWZSKREFOBJ, jref)) ? VecWzskVExpstate::MIND : VecWzskVExpstate::REGD);
...
DpchEngWzsk* PnlWzskObjList::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWzskObjList::updatePreset(
			DbsWzsk* dbswzsk
...
void PnlWzskObjList::regularize(
			DbsWzsk* dbswzsk
			, const bool notif
			, DpchEngWzsk** dpcheng
		) {
...
void PnlWzskObjList::handleRequest(
			DbsWzsk* dbswzsk
			, ReqWzsk* req
		) {
	if (req->ixVBasetype == ReqWzsk::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWzskVDpch == VecWzskVDpch::DPCHAPPWZSKOBJLISTDATA) {
		...
		} else if (req->dpchapp->ixWzskVDpch == VecWzskVDpch::DPCHAPPWZSKOBJLISTDO) {
...
void PnlWzskObjList::handleDpchAppWzskInit(
			DbsWzsk* dbswzsk
...
void PnlWzskObjList::handleDpchAppDataContiac(
			DbsWzsk* dbswzsk
			, ContIac* _contiac
			, DpchEngWzsk** dpcheng
		) {
	...
	if (has(diffitems, ContIac::NUMFTOS)) {
		if ((_contiac->numFTos >= QryWzskObjList::VecVOrd::OGR) && (_contiac->numFTos <= QryWzskObjList::VecVOrd::TIT)) {
			muteRefresh = true;
...
void PnlWzskObjList::handleDpchAppDataStgiac(
			DbsWzsk* dbswzsk
			, StgIac* _stgiac
			, DpchEngWzsk** dpcheng
		) {
...
void PnlWzskObjList::handleDpchAppDataStgiacqry(
			DbsWzsk* dbswzsk
			, QryWzskObjList::StgIac* _stgiacqry
			, DpchEngWzsk** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWzskObjList::StgIac::JNUM)) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWzskObjList::StgIac::JNUM) || (diffitems.size() > 1)) {
			muteRefresh = true;
		...
		if (has(diffitems, QryWzskObjList::StgIac::JNUM)) {
			if (refSelNew == 0) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);
			xchg->triggerIxRefCall(dbswzsk, VecWzskVCall::CALLWZSKREFPRESET, jref, VecWzskVPreset::PREWZSKREFOBJ, refSelNew);
		};
	};
...
void PnlWzskObjList::handleDpchAppDoButMinimizeClick(
			DbsWzsk* dbswzsk
			, DpchEngWzsk** dpcheng
		) {
...
void PnlWzskObjList::handleDpchAppDoButRegularizeClick(
			DbsWzsk* dbswzsk
			, DpchEngWzsk** dpcheng
		) {
...
void PnlWzskObjList::handleDpchAppDoButNewClick(
			DbsWzsk* dbswzsk
			, DpchEngWzsk** dpcheng
		) {
...
void PnlWzskObjList::handleDpchAppDoButDeleteClick(
			DbsWzsk* dbswzsk
			, DpchEngWzsk** dpcheng
		) {
...
void PnlWzskObjList::handleDpchAppDoButFilterClick(
			DbsWzsk* dbswzsk
			, DpchEngWzsk** dpcheng
		) {
...
void PnlWzskObjList::handleDpchAppDoButRefreshClick(
			DbsWzsk* dbswzsk
			, DpchEngWzsk** dpcheng
		) {
	set<uint> moditems;
...
void PnlWzskObjList::handleCall(
			DbsWzsk* dbswzsk
			, Call* call
		) {
...
bool PnlWzskObjList::handleCallWzskStatChg(
			DbsWzsk* dbswzsk
			, const ubigint jrefTrig
		) {
	bool retval = false;
