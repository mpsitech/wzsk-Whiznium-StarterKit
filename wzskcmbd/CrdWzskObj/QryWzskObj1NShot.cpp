/**
	* \file QryWzskObj1NShot.cpp
	* job handler for job QryWzskObj1NShot (implementation)
...
#include "QryWzskObj1NShot_blks.cpp"
...
/******************************************************************************
 class QryWzskObj1NShot
 ******************************************************************************/

QryWzskObj1NShot::QryWzskObj1NShot(
			XchgWzsk* xchg
			, DbsWzsk* dbswzsk
			, const ubigint jrefSup
			, const uint ixWzskVLocale
		) :
			JobWzsk(xchg, VecWzskVJob::QRYWZSKOBJ1NSHOT, jrefSup, ixWzskVLocale)
		{
...
void QryWzskObj1NShot::rerun(
	...
	dbswzsk->tblwzskqobj1nshot->removeRstByJref(jref);
	...
	sqlstr = "INSERT INTO TblWzskQObj1NShot(jref, jnum, ref)";
	...
	sqlstr = "UPDATE TblWzskQObj1NShot SET jnum = qref WHERE jref = " + to_string(jref);
	dbswzsk->executeQuery(sqlstr);
	...
	xchg->addRefClstn(VecWzskVCall::CALLWZSKSHTMOD_OBJEQ, jref, Clstn::VecVJobmask::ALL, 0, false, preRefObj);
};

void QryWzskObj1NShot::fetch(
			DbsWzsk* dbswzsk
		) {
	string sqlstr;
	...
	dbswzsk->tblwzskqobj1nshot->loadRstByJref(jref, false, rst);
	...
	if (stmgr) {
		...
		stmgr->commit();
		stmgr->unlockAccess("QryWzskObj1NShot", "fetch");
	};

	refreshJnum();
};

uint QryWzskObj1NShot::getJnumByRef(
			const ubigint ref
		) {
	uint retval = 0;
...
ubigint QryWzskObj1NShot::getRefByJnum(
			const uint jnum
		) {
	uint ref = 0;

	WzskQObj1NShot* rec = getRecByJnum(jnum);
	if (rec) ref = rec->ref;
	return ref;
};

WzskQObj1NShot* QryWzskObj1NShot::getRecByJnum(
			const uint jnum
		) {
	WzskQObj1NShot* rec = NULL;
...
void QryWzskObj1NShot::handleRequest(
			DbsWzsk* dbswzsk
			, ReqWzsk* req
		) {
...
bool QryWzskObj1NShot::handleRerun(
			DbsWzsk* dbswzsk
		) {
	bool retval = false;
	string input;
...
bool QryWzskObj1NShot::handleShow(
			DbsWzsk* dbswzsk
		) {
	bool retval = false;
	WzskQObj1NShot* rec = NULL;
...
bool QryWzskObj1NShot::handleCallWzskShtMod_objEq(
			DbsWzsk* dbswzsk
