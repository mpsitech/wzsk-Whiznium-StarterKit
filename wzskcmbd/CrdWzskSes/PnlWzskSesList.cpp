/**
	...
	* job handler for job PnlWzskSesList (implementation)
...
#include "PnlWzskSesList_blks.cpp"
#include "PnlWzskSesList_evals.cpp"
...
/******************************************************************************
 class PnlWzskSesList
 ******************************************************************************/

PnlWzskSesList::PnlWzskSesList(
			XchgWzsk* xchg
			, DbsWzsk* dbswzsk
			, const ubigint jrefSup
			, const uint ixWzskVLocale
		) :
			JobWzsk(xchg, VecWzskVJob::PNLWZSKSESLIST, jrefSup, ixWzskVLocale)
		{
	...
	QryWzskSesList::VecVOrd::fillFeed(feedFTos);

	qry = NULL;

	// IP constructor.cust1 --- INSERT

	xchg->addRefPreset(VecWzskVPreset::PREWZSKREFSEL, jref, xchg->getRefPreset(VecWzskVPreset::PREWZSKREFSES, jref));

	qry = new QryWzskSesList(xchg, dbswzsk, jref, ixWzskVLocale);

	// IP constructor.cust2 --- INSERT

	set<uint> moditems;
	statshr.ixWzskVExpstate = ((xchg->getRefPreset(VecWzskVPreset::PREWZSKREFSES, jref)) ? VecWzskVExpstate::MIND : VecWzskVExpstate::REGD);
...
DpchEngWzsk* PnlWzskSesList::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWzskSesList::updatePreset(
			DbsWzsk* dbswzsk
...
void PnlWzskSesList::regularize(
			DbsWzsk* dbswzsk
			, const bool notif
			, DpchEngWzsk** dpcheng
		) {
...
void PnlWzskSesList::handleRequest(
			DbsWzsk* dbswzsk
			, ReqWzsk* req
		) {
	if (req->ixVBasetype == ReqWzsk::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWzskVDpch == VecWzskVDpch::DPCHAPPWZSKSESLISTDATA) {
		...
		} else if (req->dpchapp->ixWzskVDpch == VecWzskVDpch::DPCHAPPWZSKSESLISTDO) {
...
void PnlWzskSesList::handleDpchAppWzskInit(
			DbsWzsk* dbswzsk
...
void PnlWzskSesList::handleDpchAppDataContiac(
			DbsWzsk* dbswzsk
			, ContIac* _contiac
			, DpchEngWzsk** dpcheng
		) {
	...
	if (has(diffitems, ContIac::NUMFTOS)) {
		if ((_contiac->numFTos >= QryWzskSesList::VecVOrd::STA) && (_contiac->numFTos <= QryWzskSesList::VecVOrd::USR)) {
			muteRefresh = true;
...
void PnlWzskSesList::handleDpchAppDataStgiac(
			DbsWzsk* dbswzsk
			, StgIac* _stgiac
			, DpchEngWzsk** dpcheng
		) {
...
void PnlWzskSesList::handleDpchAppDataStgiacqry(
			DbsWzsk* dbswzsk
			, QryWzskSesList::StgIac* _stgiacqry
			, DpchEngWzsk** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWzskSesList::StgIac::JNUM)) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWzskSesList::StgIac::JNUM) || (diffitems.size() > 1)) {
			muteRefresh = true;
		...
		if (has(diffitems, QryWzskSesList::StgIac::JNUM)) {
			if (refSelNew == 0) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);
			xchg->triggerIxRefCall(dbswzsk, VecWzskVCall::CALLWZSKREFPRESET, jref, VecWzskVPreset::PREWZSKREFSES, refSelNew);
		};
	};
...
void PnlWzskSesList::handleDpchAppDoButMinimizeClick(
			DbsWzsk* dbswzsk
			, DpchEngWzsk** dpcheng
		) {
...
void PnlWzskSesList::handleDpchAppDoButRegularizeClick(
			DbsWzsk* dbswzsk
			, DpchEngWzsk** dpcheng
		) {
...
void PnlWzskSesList::handleDpchAppDoButNewClick(
			DbsWzsk* dbswzsk
			, DpchEngWzsk** dpcheng
		) {
...
void PnlWzskSesList::handleDpchAppDoButDeleteClick(
			DbsWzsk* dbswzsk
			, DpchEngWzsk** dpcheng
		) {
...
void PnlWzskSesList::handleDpchAppDoButFilterClick(
			DbsWzsk* dbswzsk
			, DpchEngWzsk** dpcheng
		) {
...
void PnlWzskSesList::handleDpchAppDoButRefreshClick(
			DbsWzsk* dbswzsk
			, DpchEngWzsk** dpcheng
		) {
	set<uint> moditems;
...
void PnlWzskSesList::handleCall(
			DbsWzsk* dbswzsk
			, Call* call
		) {
...
bool PnlWzskSesList::handleCallWzskStatChg(
			DbsWzsk* dbswzsk
			, const ubigint jrefTrig
		) {
	bool retval = false;
