/**
	...
	* job handler for job PnlWzskUsrAAccess (implementation)
...
#include "PnlWzskUsrAAccess_blks.cpp"
#include "PnlWzskUsrAAccess_evals.cpp"
...
/******************************************************************************
 class PnlWzskUsrAAccess
 ******************************************************************************/

PnlWzskUsrAAccess::PnlWzskUsrAAccess(
			XchgWzsk* xchg
			, DbsWzsk* dbswzsk
			, const ubigint jrefSup
			, const uint ixWzskVLocale
		) :
			JobWzsk(xchg, VecWzskVJob::PNLWZSKUSRAACCESS, jrefSup, ixWzskVLocale)
		{
	...
	qry = new QryWzskUsrAAccess(xchg, dbswzsk, jref, ixWzskVLocale);
...
DpchEngWzsk* PnlWzskUsrAAccess::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWzskUsrAAccess::refresh(
			DbsWzsk* dbswzsk
			, set<uint>& moditems
			, const bool unmute
		) {
...
void PnlWzskUsrAAccess::updatePreset(
			DbsWzsk* dbswzsk
...
void PnlWzskUsrAAccess::handleRequest(
			DbsWzsk* dbswzsk
			, ReqWzsk* req
		) {
	if (req->ixVBasetype == ReqWzsk::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWzskVDpch == VecWzskVDpch::DPCHAPPWZSKUSRAACCESSDATA) {
		...
		} else if (req->dpchapp->ixWzskVDpch == VecWzskVDpch::DPCHAPPWZSKUSRAACCESSDO) {
...
void PnlWzskUsrAAccess::handleDpchAppWzskInit(
			DbsWzsk* dbswzsk
...
void PnlWzskUsrAAccess::handleDpchAppDataStgiac(
			DbsWzsk* dbswzsk
			, StgIac* _stgiac
			, DpchEngWzsk** dpcheng
		) {
...
void PnlWzskUsrAAccess::handleDpchAppDataStgiacqry(
			DbsWzsk* dbswzsk
			, QryWzskUsrAAccess::StgIac* _stgiacqry
			, DpchEngWzsk** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WzskQUsrAAccess* recSelNew = NULL;

	WzskAMUserAccess* _recUsrAacc = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWzskUsrAAccess::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWzskUsrAAccess::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswzsk);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWzskUsrAAccess::StgIac::JNUM)) {
			if (!recSelNew) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

			recUsrAacc = WzskAMUserAccess();

			if (recSelNew) {
				if (dbswzsk->tblwzskamuseraccess->loadRecByRef(recSelNew->ref, &_recUsrAacc)) {
...
void PnlWzskUsrAAccess::handleDpchAppDoButNewClick(
			DbsWzsk* dbswzsk
			, DpchEngWzsk** dpcheng
		) {
...
void PnlWzskUsrAAccess::handleDpchAppDoButDuplicateClick(
			DbsWzsk* dbswzsk
			, DpchEngWzsk** dpcheng
		) {
...
void PnlWzskUsrAAccess::handleDpchAppDoButDeleteClick(
			DbsWzsk* dbswzsk
			, DpchEngWzsk** dpcheng
		) {
...
void PnlWzskUsrAAccess::handleDpchAppDoButRefreshClick(
			DbsWzsk* dbswzsk
			, DpchEngWzsk** dpcheng
		) {
	set<uint> moditems;
...
void PnlWzskUsrAAccess::handleCall(
			DbsWzsk* dbswzsk
			, Call* call
		) {
...
bool PnlWzskUsrAAccess::handleCallWzskStatChg(
			DbsWzsk* dbswzsk
			, const ubigint jrefTrig
		) {
	bool retval = false;
