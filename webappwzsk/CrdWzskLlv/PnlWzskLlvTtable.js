...
function minimize() {
	...
	getCrdwnd().changeHeight("Ttable", 30);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("Ttable_side").setAttribute("height", "30");
	doc.getElementById("Ttable_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("Ttable_side").src = "./PnlWzskLlvTtable_aside.html";
	doc.getElementById("Ttable_cont").src = "./PnlWzskLlvTtable_a.html";
};

function regularize() {
	// change content (container heights in refreshBD)
	doc.getElementById("Ttable_side").src = "./PnlWzskLlvTtable_bside.html";
	doc.getElementById("Ttable_cont").src = "./PnlWzskLlvTtable_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("Ttable_side").contentDocument;
	contdoc = doc.getElementById("Ttable_cont").contentDocument;
	hdrdoc = null;
	contcontdoc = null;
	ftrdoc = null;

	initCpt(contdoc, "Cpt", retrieveTi(srcdoc, "TagWzskLlvTtable", "Cpt"));
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("Ttable_side").contentDocument;
	contdoc = doc.getElementById("Ttable_cont").contentDocument;
	hdrdoc = contdoc.getElementById("Ttable_hdr").contentDocument;
	contcontdoc = contdoc.getElementById("Ttable_cont").contentDocument;
...
function init() {
	var srefIxWzskVExpstate = retrieveSi(srcdoc, "StatShrWzskLlvTtable", "srefIxWzskVExpstate");
...
function refreshBD(bNotD) {
	...
	getCrdwnd().changeHeight("Ttable", height+31);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+31));
	doc.getElementById("Ttable_side").setAttribute("height", "" + (height+31));
	doc.getElementById("Ttable_cont").setAttribute("height", "" + (height+31));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+31-30));
	contdoc.getElementById("tdCont").setAttribute("height", "" + height);
	contdoc.getElementById("Ttable_cont").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("Ttable_src").contentDocument;
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWzskLlvTtableDo", scrJref, ctlsref + "Click");
...
function handleSldJpleftClick(_doc, ctlsref, shr, log, _rast, evt) {
	...
	if (shr) oldVal = parseFloat(retrieveCi(srcdoc, "ContIacWzskLlvTtable", ctlsref));
	else oldVal = parseFloat(_doc.getElementById(ctlsref + "Val").value);
	min = parseFloat(retrieveSi(srcdoc, "StatShrWzskLlvTtable", ctlsref + "Min"));
	max = parseFloat(retrieveSi(srcdoc, "StatShrWzskLlvTtable", ctlsref + "Max"));

	if (_rast) rast = parseFloat(retrieveSi(srcdoc, "StatShrWzskLlvTtable", ctlsref + "Rast"));
	...
	if (shr) {
		setCi(srcdoc, "ContIacWzskLlvTtable", ctlsref, "" + val);

		var str = serializeDpchAppData(srcdoc, "DpchAppWzskLlvTtableData", scrJref, "ContIacWzskLlvTtable");
		sendReq(str, doc, handleDpchAppDataDoReply);
...
function handleSldMove(_doc, ctlsref, shr, log, _rast, evt) {
	...
	if (shr) setCi(srcdoc, "ContIacWzskLlvTtable", ctlsref, "" + val);
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWzskLlvTtableData", "ContIacWzskLlvTtable", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngWzskLlvTtableData", "ContInfWzskLlvTtable", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWzskLlvTtableData", "StatShrWzskLlvTtable", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWzskLlvTtableData", "TagWzskLlvTtable", srcdoc)) mask.push("tag");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWzskLlvTtableData") {
		var oldSrefIxWzskVExpstate = retrieveSi(srcdoc, "StatShrWzskLlvTtable", "srefIxWzskVExpstate");
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWzskLlvTtableData") {
				mergeDpchEngData(dom);
				init();

				getCrdwnd().setInitdone("Ttable");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWzskLlvTtableData") {
				var oldSrefIxWzskVExpstate = retrieveSi(srcdoc, "StatShrWzskLlvTtable", "srefIxWzskVExpstate");
