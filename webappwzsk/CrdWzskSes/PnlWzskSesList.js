/**
  * \file PnlWzskSesList.js
  * web client functionality for panel PnlWzskSesList
...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlWzskSesList_aside.html";
	doc.getElementById("List_cont").src = "./PnlWzskSesList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlWzskSesList_bside.html";
	doc.getElementById("List_cont").src = "./PnlWzskSesList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWzskSesList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWzskSesList", "Trs"));
...
function init() {
	var srefIxWzskVExpstate = retrieveSi(srcdoc, "StatShrWzskSesList", "srefIxWzskVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWzskSesList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWzskSesList", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWzskSesList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzskSesList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzskSesList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWzskSesList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWzskSesList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWzskSesList", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWzskSesList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWzskSesList", "ncol"));

		var TcoUsrWidth = parseInt(retrieveSi(srcdoc, "StgIacWzskSesList", "TcoUsrWidth"));
		var TcoStaWidth = parseInt(retrieveSi(srcdoc, "StgIacWzskSesList", "TcoStaWidth"));
		var TcoStoWidth = parseInt(retrieveSi(srcdoc, "StgIacWzskSesList", "TcoStoWidth"));
		var TcoIpWidth = parseInt(retrieveSi(srcdoc, "StgIacWzskSesList", "TcoIpWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacWzskSesList", "numFTos"));
		...
		for (i = firstcol, ix = 0; i <= ncol; i++) {
			...
			if (i == 1) {
				doc.cols.push("TcoUsr"); doc.widths.push(TcoUsrWidth); doc.fs.push("usr"); doc.tos.push(2); wtot += TcoUsrWidth;
			} else if (i == 2) {
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagWzskSesList", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWzskSesList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWzskQSesList
		rownode = getRownode(srcdoc, "ListWzskQSesList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWzskSesList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWzskSesList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWzskSesList", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WzskSesList", "ListWzskQSesList", retrieveTi(srcdoc, "TagWzskSesList", "TxtRecord1"), retrieveTi(srcdoc, "TagWzskSesList", "TxtRecord2"),
				["Usr","Sta","Sto","Ip"],
				["usr","sta","sto","ip"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWzskSesList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzskSesList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWzskSesList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWzskSesList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWzskSesList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWzskSesListData", scrJref, "StgIacQryWzskSesList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWzskSesList", "firstcol"));

	setSi(srcdoc, "StatAppQryWzskSesList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWzskSesList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWzskSesListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacWzskSesList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacWzskSesList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppWzskSesListData", scrJref, "ContIacWzskSesList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWzskSesListData", "ContIacWzskSesList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngWzskSesListData", "ContInfWzskSesList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWzskSesListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWzskSesListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngWzskSesListData", "StatShrWzskSesList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWzskSesListData", "StgIacWzskSesList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWzskSesListData", "TagWzskSesList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWzskSesListData", "ListWzskQSesList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWzskSesListData", "StatAppQryWzskSesList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWzskSesListData", "StatShrQryWzskSesList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWzskSesListData", "StgIacQryWzskSesList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngWzskSesListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWzskSesListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWzskSesListData") {
				mask = mergeDpchEngData(dom);
