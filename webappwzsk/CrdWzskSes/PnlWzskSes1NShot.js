/**
  * \file PnlWzskSes1NShot.js
  * web client functionality for panel PnlWzskSes1NShot
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWzskSes1NShot", "srefIxWzskVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWzskSes1NShot", "srefIxWzskVExpstate", "mind");

	getRecwnd().changeHeight("1NShot", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("1NShot_side").setAttribute("height", "30");
	doc.getElementById("1NShot_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("1NShot_side").src = "./PnlWzskSes1NShot_aside.html";
	doc.getElementById("1NShot_cont").src = "./PnlWzskSes1NShot_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWzskSes1NShot", "srefIxWzskVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWzskSes1NShot", "srefIxWzskVExpstate", "regd");

	getRecwnd().changeHeight("1NShot", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("1NShot_side").setAttribute("height", "255");
	doc.getElementById("1NShot_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("1NShot_side").src = "./PnlWzskSes1NShot_bside.html";
	doc.getElementById("1NShot_cont").src = "./PnlWzskSes1NShot_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("1NShot_side").contentDocument;
	contdoc = doc.getElementById("1NShot_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("1NShot_side").contentDocument;
	contdoc = doc.getElementById("1NShot_cont").contentDocument;
	hdrdoc = contdoc.getElementById("1NShot_hdr").contentDocument;
	tbldoc = contdoc.getElementById("1NShot_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("1NShot_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWzskSes1NShot", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWzskSes1NShot", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWzskVExpstate = retrieveSi(srcdoc, "StatAppWzskSes1NShot", "srefIxWzskVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWzskSes1NShot", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWzskSes1NShot", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWzskSes1NShot", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzskSes1NShot", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzskSes1NShot", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWzskSes1NShot", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWzskSes1NShot", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWzskSes1NShot", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWzskSes1NShot", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWzskSes1NShot", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWzskSes1NShot", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWzskSes1NShot", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWzskSes1NShot", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWzskQSes1NShot
		rownode = getRownode(srcdoc, "ListWzskQSes1NShot", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWzskSes1NShot", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWzskSes1NShot", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWzskSes1NShot", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("1NShot", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("1NShot_side").setAttribute("height", "" + (height+55));
	doc.getElementById("1NShot_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("1NShot_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("1NShot_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WzskSes1NShot", "ListWzskQSes1NShot", retrieveTi(srcdoc, "TagWzskSes1NShot", "TxtRecord1"), retrieveTi(srcdoc, "TagWzskSes1NShot", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWzskSes1NShot", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzskSes1NShot", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWzskSes1NShot", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWzskSes1NShot", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWzskSes1NShot", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWzskSes1NShotData", scrJref, "StgIacQryWzskSes1NShot");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWzskSes1NShot", "srefIxWzskVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWzskSes1NShot", "firstcol"));

	setSi(srcdoc, "StatAppQryWzskSes1NShot", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWzskSes1NShot", "srefIxWzskVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWzskSes1NShot", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWzskSes1NShotDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWzskSes1NShotData", "ContInfWzskSes1NShot", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWzskSes1NShotData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWzskSes1NShotData", "StatAppWzskSes1NShot", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWzskSes1NShotData", "StatShrWzskSes1NShot", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWzskSes1NShotData", "StgIacWzskSes1NShot", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWzskSes1NShotData", "TagWzskSes1NShot", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWzskSes1NShotData", "ListWzskQSes1NShot", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWzskSes1NShotData", "StatAppQryWzskSes1NShot", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWzskSes1NShotData", "StatShrQryWzskSes1NShot", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWzskSes1NShotData", "StgIacQryWzskSes1NShot", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWzskSes1NShotData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWzskSes1NShotData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("1NShot");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWzskSes1NShotData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
