/**
  * \file PnlWzskUsgMNUser.js
  * web client functionality for panel PnlWzskUsgMNUser
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWzskUsgMNUser", "srefIxWzskVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWzskUsgMNUser", "srefIxWzskVExpstate", "mind");
	...
	// change content
	doc.getElementById("MNUser_side").src = "./PnlWzskUsgMNUser_aside.html";
	doc.getElementById("MNUser_cont").src = "./PnlWzskUsgMNUser_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWzskUsgMNUser", "srefIxWzskVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWzskUsgMNUser", "srefIxWzskVExpstate", "regd");
	...
	// change content
	doc.getElementById("MNUser_side").src = "./PnlWzskUsgMNUser_bside.html";
	doc.getElementById("MNUser_cont").src = "./PnlWzskUsgMNUser_b.html";
};
...
function initBD(bNotD) {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWzskUsgMNUser", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWzskUsgMNUser", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWzskVExpstate = retrieveSi(srcdoc, "StatAppWzskUsgMNUser", "srefIxWzskVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWzskUsgMNUser", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWzskUsgMNUser", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWzskUsgMNUser", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzskUsgMNUser", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzskUsgMNUser", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWzskUsgMNUser", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWzskUsgMNUser", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWzskUsgMNUser", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWzskUsgMNUser", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWzskUsgMNUser", "ncol"));

		var TcoMrefWidth = parseInt(retrieveSi(srcdoc, "StgIacWzskUsgMNUser", "TcoMrefWidth"));
		var TcoUlvWidth = parseInt(retrieveSi(srcdoc, "StgIacWzskUsgMNUser", "TcoUlvWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWzskUsgMNUser", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWzskUsgMNUser", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWzskQUsgMNUser
		rownode = getRownode(srcdoc, "ListWzskQUsgMNUser", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWzskUsgMNUser", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWzskUsgMNUser", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWzskUsgMNUser", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WzskUsgMNUser", "ListWzskQUsgMNUser", retrieveTi(srcdoc, "TagWzskUsgMNUser", "TxtRecord1"), retrieveTi(srcdoc, "TagWzskUsgMNUser", "TxtRecord2"),
				["Mref","Ulv",""],
				["mref","ulv","ulv2"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWzskUsgMNUser", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzskUsgMNUser", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWzskUsgMNUser", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWzskUsgMNUser", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWzskUsgMNUser", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWzskUsgMNUserData", scrJref, "StgIacQryWzskUsgMNUser");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWzskUsgMNUser", "srefIxWzskVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWzskUsgMNUser", "firstcol"));

	setSi(srcdoc, "StatAppQryWzskUsgMNUser", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWzskUsgMNUser", "srefIxWzskVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWzskUsgMNUser", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWzskUsgMNUserDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWzskUsgMNUserData", "ContInfWzskUsgMNUser", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWzskUsgMNUserData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWzskUsgMNUserData", "StatAppWzskUsgMNUser", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWzskUsgMNUserData", "StatShrWzskUsgMNUser", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWzskUsgMNUserData", "StgIacWzskUsgMNUser", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWzskUsgMNUserData", "TagWzskUsgMNUser", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWzskUsgMNUserData", "ListWzskQUsgMNUser", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWzskUsgMNUserData", "StatAppQryWzskUsgMNUser", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWzskUsgMNUserData", "StatShrQryWzskUsgMNUser", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWzskUsgMNUserData", "StgIacQryWzskUsgMNUser", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWzskUsgMNUserData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWzskUsgMNUserData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWzskUsgMNUserData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
