...
function minimize(updh) {
	...
	setSi(srcdoc, "StatAppWzskShtAPar", "srefIxWzskVExpstate", "mind");
	...
	// change content
	doc.getElementById("APar_side").src = "./PnlWzskShtAPar_aside.html";
	doc.getElementById("APar_cont").src = "./PnlWzskShtAPar_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWzskShtAPar", "srefIxWzskVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWzskShtAPar", "srefIxWzskVExpstate", "regd");
	...
	// change content
	doc.getElementById("APar_side").src = "./PnlWzskShtAPar_bside.html";
	doc.getElementById("APar_cont").src = "./PnlWzskShtAPar_b.html";
};
...
function initBD(bNotD) {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWzskShtAPar", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWzskShtAPar", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWzskVExpstate = retrieveSi(srcdoc, "StatAppWzskShtAPar", "srefIxWzskVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWzskShtAPar", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWzskShtAPar", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWzskShtAPar", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzskShtAPar", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzskShtAPar", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWzskShtAPar", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWzskShtAPar", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWzskShtAPar", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWzskShtAPar", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWzskShtAPar", "ncol"));

		var TcoKeyWidth = parseInt(retrieveSi(srcdoc, "StgIacWzskShtAPar", "TcoKeyWidth"));
		var TcoValWidth = parseInt(retrieveSi(srcdoc, "StgIacWzskShtAPar", "TcoValWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWzskShtAPar", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWzskShtAPar", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWzskQShtAPar
		rownode = getRownode(srcdoc, "ListWzskQShtAPar", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWzskShtAPar", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWzskShtAPar", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWzskShtAPar", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WzskShtAPar", "ListWzskQShtAPar", retrieveTi(srcdoc, "TagWzskShtAPar", "TxtRecord1"), retrieveTi(srcdoc, "TagWzskShtAPar", "TxtRecord2"),
				["Key","","Val"],
				["key","key2","val"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWzskShtAPar", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzskShtAPar", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWzskShtAPar", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWzskShtAPar", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWzskShtAPar", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWzskShtAParData", scrJref, "StgIacQryWzskShtAPar");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWzskShtAPar", "srefIxWzskVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWzskShtAPar", "firstcol"));

	setSi(srcdoc, "StatAppQryWzskShtAPar", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWzskShtAPar", "srefIxWzskVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWzskShtAPar", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWzskShtAParDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWzskShtAParData", "ContInfWzskShtAPar", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWzskShtAParData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWzskShtAParData", "StatAppWzskShtAPar", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWzskShtAParData", "StatShrWzskShtAPar", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWzskShtAParData", "StgIacWzskShtAPar", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWzskShtAParData", "TagWzskShtAPar", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWzskShtAParData", "ListWzskQShtAPar", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWzskShtAParData", "StatAppQryWzskShtAPar", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWzskShtAParData", "StatShrQryWzskShtAPar", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWzskShtAParData", "StgIacQryWzskShtAPar", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWzskShtAParData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWzskShtAParData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWzskShtAParData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
