/**
  * \file CrdWzskSht.js
  * web client functionality for card CrdWzskSht
...
function getInitdone(pnlshort) {
	return(retrieveSi(srcdoc, "StatAppWzskSht", "initdone" + pnlshort) == "true");
};

function setInitdone(pnlshort) {
	setSi(srcdoc, "StatAppWzskSht", "initdone" + pnlshort, "true");

	checkInitdone();
};

function checkInitdone() {
	var initdoneHeadbar = (retrieveSi(srcdoc, "StatAppWzskSht", "initdoneHeadbar") == "true");
	var initdoneList = (retrieveSi(srcdoc, "StatAppWzskSht", "initdoneList") == "true");
	var initdoneRec = (retrieveSi(srcdoc, "StatAppWzskSht", "initdoneRec") == "true");

	if (!initdoneHeadbar) {
		doc.getElementById("Headbar").src = "./PnlWzskShtHeadbar.html?scrJref=" + scrJrefHeadbar;
	} else if (!initdoneList) {
		doc.getElementById("List").src = "./PnlWzskShtList.html?scrJref=" + scrJrefList;
	} else if (!initdoneRec) {
		doc.getElementById("Rec").src = "./PnlWzskShtRec.html?scrJref=" + scrJrefRec;
	} else {
		doc.initdone = true;
...
function toggleMenu(short, left, width, height) {
	var shortMenu = retrieveSi(srcdoc, "StatAppWzskSht", "shortMenu");
...
function showMenu(short, left, width, height) {
	...
	setSi(srcdoc, "StatAppWzskSht", "shortMenu", short);
	setSi(srcdoc, "StatAppWzskSht", "widthMenu", "" + (width-3));
...
function hideMenu(short) {
	...
	setSi(srcdoc, "StatAppWzskSht", "shortMenu", "");
...
function initMenApp() {
	...
	mendoc.getElementById("colCont").setAttribute("width", retrieveSi(srcdoc, "StatAppWzskSht", "widthMenu"));

	setTextContent(mendoc, mendoc.getElementById("MitAppAbt"), retrieveTi(srcdoc, "TagWzskSht", "MitAppAbt"));
	setTextContent(mendoc, mendoc.getElementById("MrlAppHlp"), retrieveTi(srcdoc, "TagWzskSht", "MrlAppHlp"));
};
...
function initMenCrd() {
	...
	setTextContent(mendoc, mendoc.getElementById("MtxCrdSht"), retrieveCi(srcdoc, "ContInfWzskSht", "MtxCrdSht"));
};
...
function refresh() {
	...
	doc.title = retrieveCi(srcdoc, "ContInfWzskSht", "MtxCrdSht") + " - Whiznium StarterKit v0.1.37";
};
...
function handleUnload() {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWzskShtDo", scrJref, "close");
	sendReq(str, doc, null, true);
};

function handleMitClick(menshort, consref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWzskShtDo", scrJref, consref + "Click");
	sendReq(str, doc, handleDpchAppDoReply);
...
function handleMrlClick(menshort, consref) {
	window.open(retrieveCi(srcdoc, "ContInfWzskSht", consref), "_blank");
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWzskShtData", "ContInfWzskSht", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWzskShtData", "FeedFSge", srcdoc)) mask.push("feedFSge");
	if (updateSrcblock(dom, "DpchEngWzskShtData", "StatAppWzskSht", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWzskShtData", "StatShrWzskSht", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWzskShtData", "TagWzskSht", srcdoc)) mask.push("tag");
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWzskShtData") {
				mergeDpchEngData(dom);

				// establish sub-panel scrJref's as global variables, and start sub-panel load sequence
				scrJrefHeadbar = retrieveSi(srcdoc, "StatShrWzskSht", "scrJrefHeadbar");
				scrJrefList = retrieveSi(srcdoc, "StatShrWzskSht", "scrJrefList");
				scrJrefRec = retrieveSi(srcdoc, "StatShrWzskSht", "scrJrefRec");
		...
		if (blk) {
			...
			if (_scrJref != "") {
				if (_scrJref == scrJref) {
					...
					} else if (blk.nodeName == "DpchEngWzskShtData") {
						mergeDpchEngData(dom);
						refresh();
					};
...
function restartNotify() {
	var srefIxWzskVReqitmode = retrieveSi(srcdoc, "StatAppWzskSht", "srefIxWzskVReqitmode");

	if (srefIxWzskVReqitmode != "notify") {
		if (doc.timeout) window.clearTimeout(doc.timeout);

		srefIxWzskVReqitmode = "notify";
		setSi(srcdoc, "StatAppWzskSht", "srefIxWzskVReqitmode", srefIxWzskVReqitmode);
...
function iterateReqit(ev) {
	...
	var srefIxWzskVReqitmode = retrieveSi(srcdoc, "StatAppWzskSht", "srefIxWzskVReqitmode");
	var latency = parseInt(retrieveSi(srcdoc, "StatAppWzskSht", "latency"));
