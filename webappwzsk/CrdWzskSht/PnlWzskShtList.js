...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlWzskShtList_aside.html";
	doc.getElementById("List_cont").src = "./PnlWzskShtList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlWzskShtList_bside.html";
	doc.getElementById("List_cont").src = "./PnlWzskShtList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWzskShtList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWzskShtList", "Trs"));
...
function init() {
	var srefIxWzskVExpstate = retrieveSi(srcdoc, "StatShrWzskShtList", "srefIxWzskVExpstate");
...
function refreshA() {
	var TxtFor = retrieveCi(srcdoc, "ContInfWzskShtList", "TxtFor");

	setTextContent(contdoc, contdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagWzskShtList", "TxtFor"), TxtFor));

	if (TxtFor == "") {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWzskShtList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWzskShtList", "ntot") + ")");
		clearElem(contdoc.getElementById("TxtPre"));
	} else {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWzskShtList", "Cpt"));
		setTextContent(contdoc, contdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfWzskShtList", "TxtPre") + " (" + retrieveSi(srcdoc, "StatShrQryWzskShtList", "ntot") + ")");
	};
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWzskShtList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzskShtList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzskShtList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWzskShtList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWzskShtList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWzskShtList", "ntot"));
	...
	// header row
	setTextContent(hdrdoc, hdrdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagWzskShtList", "TxtFor"), TxtFor));

	if (TxtFor == "") clearElem(hdrdoc.getElementById("TxtPre"));
	else setTextContent(hdrdoc, hdrdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfWzskShtList", "TxtPre"));

	// update column characteristics
	if (chgcol) {
		// obtain column (header) characteristics
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWzskShtList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWzskShtList", "ncol"));

		var TcoSesWidth = parseInt(retrieveSi(srcdoc, "StgIacWzskShtList", "TcoSesWidth"));
		var TcoObjWidth = parseInt(retrieveSi(srcdoc, "StgIacWzskShtList", "TcoObjWidth"));
		var TcoStaWidth = parseInt(retrieveSi(srcdoc, "StgIacWzskShtList", "TcoStaWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacWzskShtList", "numFTos"));
		...
		for (i = firstcol, ix = 0; i <= ncol; i++) {
			...
			if (i == 1) {
				doc.cols.push("TcoSes"); doc.widths.push(TcoSesWidth); doc.fs.push("ses"); doc.tos.push(2); wtot += TcoSesWidth;
			} else if (i == 2) {
				doc.cols.push("TcoObj"); doc.widths.push(TcoObjWidth); doc.fs.push("obj"); doc.tos.push(3); wtot += TcoObjWidth;
			} else if (i == 3) {
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagWzskShtList", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWzskShtList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWzskQShtList
		rownode = getRownode(srcdoc, "ListWzskQShtList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWzskShtList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWzskShtList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWzskShtList", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WzskShtList", "ListWzskQShtList", retrieveTi(srcdoc, "TagWzskShtList", "TxtRecord1"), retrieveTi(srcdoc, "TagWzskShtList", "TxtRecord2"),
				["Ses","Obj","Sta"],
				["ses","obj","sta"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWzskShtList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzskShtList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWzskShtList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWzskShtList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWzskShtList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWzskShtListData", scrJref, "StgIacQryWzskShtList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWzskShtList", "firstcol"));

	setSi(srcdoc, "StatAppQryWzskShtList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWzskShtList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWzskShtListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacWzskShtList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacWzskShtList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppWzskShtListData", scrJref, "ContIacWzskShtList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWzskShtListData", "ContIacWzskShtList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngWzskShtListData", "ContInfWzskShtList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWzskShtListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWzskShtListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngWzskShtListData", "StatShrWzskShtList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWzskShtListData", "StgIacWzskShtList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWzskShtListData", "TagWzskShtList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWzskShtListData", "ListWzskQShtList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWzskShtListData", "StatAppQryWzskShtList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWzskShtListData", "StatShrQryWzskShtList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWzskShtListData", "StgIacQryWzskShtList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngWzskShtListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWzskShtListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWzskShtListData") {
				mask = mergeDpchEngData(dom);
