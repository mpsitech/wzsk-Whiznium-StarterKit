function getInitdone(pnlshort) {
	return(retrieveSi(srcdoc, "StatAppWzskHwc", "initdone" + pnlshort) == "true");
};

function setInitdone(pnlshort) {
	setSi(srcdoc, "StatAppWzskHwc", "initdone" + pnlshort, "true");

	checkInitdone();
};

function checkInitdone() {
	var initdoneConfig = (retrieveSi(srcdoc, "StatAppWzskHwc", "initdoneConfig") == "true");
	var initdoneAsys = (retrieveSi(srcdoc, "StatAppWzskHwc", "initdoneAsys") == "true");
	var initdoneDebug = (retrieveSi(srcdoc, "StatAppWzskHwc", "initdoneDebug") == "true");
	var initdoneHeadbar = (retrieveSi(srcdoc, "StatAppWzskHwc", "initdoneHeadbar") == "true");

	if (!initdoneConfig) {
		doc.getElementById("Config").src = "./PnlWzskHwcConfig.html?scrJref=" + scrJrefConfig;
	} else if (!initdoneAsys) {
		doc.getElementById("Asys").src = "./PnlWzskHwcAsys.html?scrJref=" + scrJrefAsys;
	} else if (!initdoneDebug) {
		doc.getElementById("Debug").src = "./PnlWzskHwcDebug.html?scrJref=" + scrJrefDebug;
	} else if (!initdoneHeadbar) {
		doc.getElementById("Headbar").src = "./PnlWzskHwcHeadbar.html?scrJref=" + scrJrefHeadbar;
	} else {
		doc.initdone = true;
...
function getHeight() {
	...
	height += 10 + parseInt(doc.getElementById("tdAsys").getAttribute("height")) + 8;
...
function toggleMenu(short, left, width, height) {
	var shortMenu = retrieveSi(srcdoc, "StatAppWzskHwc", "shortMenu");
...
function showMenu(short, left, width, height) {
	...
	setSi(srcdoc, "StatAppWzskHwc", "shortMenu", short);
	setSi(srcdoc, "StatAppWzskHwc", "widthMenu", "" + (width-3));
...
function hideMenu(short) {
	...
	setSi(srcdoc, "StatAppWzskHwc", "shortMenu", "");
...
function initMenApp() {
	...
	mendoc.getElementById("colCont").setAttribute("width", retrieveSi(srcdoc, "StatAppWzskHwc", "widthMenu"));

	setTextContent(mendoc, mendoc.getElementById("MitAppAbt"), retrieveTi(srcdoc, "TagWzskHwc", "MitAppAbt"));
	setTextContent(mendoc, mendoc.getElementById("MrlAppHlp"), retrieveTi(srcdoc, "TagWzskHwc", "MrlAppHlp"));
};
...
function changeHeight(pnlshort, height) {
	...
	} else if (pnlshort == "Asys") {
		doc.getElementById("tdAsys").setAttribute("height", "" + height);
		doc.getElementById("Asys").setAttribute("height", "" + height);
...
function handleUnload() {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWzskHwcDo", scrJref, "close");
	sendReq(str, doc, null, true);
};

function handleMitClick(menshort, consref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWzskHwcDo", scrJref, consref + "Click");
	sendReq(str, doc, handleDpchAppDoReply);
...
function handleMrlClick(menshort, consref) {
	window.open(retrieveCi(srcdoc, "ContInfWzskHwc", consref), "_blank");
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWzskHwcData", "ContInfWzskHwc", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWzskHwcData", "FeedFSge", srcdoc)) mask.push("feedFSge");
	if (updateSrcblock(dom, "DpchEngWzskHwcData", "StatAppWzskHwc", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWzskHwcData", "StatShrWzskHwc", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWzskHwcData", "TagWzskHwc", srcdoc)) mask.push("tag");
...
function handleDpchEngSub(_scrJref, dom, dpch) {
	...
	} else if (_scrJref == scrJrefAsys) {
		doc.getElementById("Asys").contentWindow.handleDpchEng(dom, dpch);
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWzskHwcData") {
				mergeDpchEngData(dom);

				// establish sub-panel scrJref's as global variables, and start sub-panel load sequence
				scrJrefConfig = retrieveSi(srcdoc, "StatShrWzskHwc", "scrJrefConfig");
				scrJrefAsys = retrieveSi(srcdoc, "StatShrWzskHwc", "scrJrefAsys");
				scrJrefDebug = retrieveSi(srcdoc, "StatShrWzskHwc", "scrJrefDebug");
				scrJrefHeadbar = retrieveSi(srcdoc, "StatShrWzskHwc", "scrJrefHeadbar");
		...
		if (blk) {
			...
			if (_scrJref != "") {
				if (_scrJref == scrJref) {
					...
					} else if (blk.nodeName == "DpchEngWzskHwcData") {
						mergeDpchEngData(dom);
						refresh();
					};
...
function restartNotify() {
	var srefIxWzskVReqitmode = retrieveSi(srcdoc, "StatAppWzskHwc", "srefIxWzskVReqitmode");

	if (srefIxWzskVReqitmode != "notify") {
		if (doc.timeout) window.clearTimeout(doc.timeout);

		srefIxWzskVReqitmode = "notify";
		setSi(srcdoc, "StatAppWzskHwc", "srefIxWzskVReqitmode", srefIxWzskVReqitmode);
...
function iterateReqit(ev) {
	...
	var srefIxWzskVReqitmode = retrieveSi(srcdoc, "StatAppWzskHwc", "srefIxWzskVReqitmode");
	var latency = parseInt(retrieveSi(srcdoc, "StatAppWzskHwc", "latency"));
