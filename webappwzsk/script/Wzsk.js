...
function serializeDpchAppAlert(scrJref, numFMcb) {
	...
	dpch = doc.createElement("DpchAppWzskAlert");
...
function serializeDpchAppData(srcdoc, sref, scrJref, blk) {
	...
	elem = retrieveBlock(srcdoc, "//wzsk:" + blk);
	if (elem) {
...
function retrieveSi(srcdoc, blk, tag) {
	return(retrieveValue(srcdoc, "//wzsk:" + blk + "/wzsk:Si[@sref='" + tag + "']"));
};

function retrieveTi(srcdoc, blk, tag) {
	return(retrieveValue(srcdoc, "//wzsk:" + blk + "/wzsk:Ti[@sref='" + tag + "']"));
};

function retrieveCi(srcdoc, blk, tag) {
	return(retrieveValue(srcdoc, "//wzsk:" + blk + "/wzsk:Ci[@sref='" + tag + "']"));
};
...
function setSi(srcdoc, blk, tag, val) {
	...
	resnode = getNode(srcdoc, "//wzsk:" + blk + "/wzsk:Si[@sref='" + tag + "']");
	if (resnode) {
...
function setCi(srcdoc, blk, tag, val) {
	...
	resnode = getNode(srcdoc, "//wzsk:" + blk + "/wzsk:Ci[@sref='" + tag + "']");
	if (resnode) {
		if (val.replace) txt = srcdoc.createTextNode(val.replace(/[\u0022\u0026\u003c\u003e\u0080-\u00ff]/gi, function(match, grp) {return('\\u00' + match.charCodeAt(0).toString(16));}));
		else txt = srcdoc.createTextNode(val);
...
function replaceSrcblock(blk, blksref, srcdoc) {
	...
	if (blk) {
		resnode = getNode(srcdoc, "//wzsk:src");

		blkold = getNode(srcdoc, "//wzsk:" + blksref);
...
function updateSrcblock(dpchdom, dpchsref, blksref, srcdoc) {
	...
	blk = getNode(dpchdom, "//wzsk:" + dpchsref + "/wzsk:" + blksref);
...
function nsResolver(prefix) {
	return "http://www.mpsitech.com/wzsk";
};

function getFeedSize(srcdoc, blk) {
	return(srcdoc.evaluate("count(//wzsk:" + blk + "/wzsk:Fi)", srcdoc, nsResolver, XPathResult.NUMBER_TYPE, null).numberValue);
};

function retrieveFeedNumBySref(srcdoc, blk, sref) {
	return(parseInt(retrieveValue(srcdoc, "//wzsk:" + blk + "/wzsk:Fi[wzsk:sref='" + sref + "']/@num")));
};

function retrieveFeedSelByNum(srcdoc, blk, num) {
	return(retrieveValue(srcdoc, "//wzsk:" + blk + "/wzsk:Fi[@num='" + num + "']/wzsk:sel") == "true");
};

function retrieveFeedSrefByNum(srcdoc, blk, num) {
	return(retrieveValue(srcdoc, "//wzsk:" + blk + "/wzsk:Fi[@num='" + num + "']/wzsk:sref"));
};

function retrieveFeedTitleByNumI(srcdoc, blk, num, i) {
	if (i) {
		if (i == 2) return(retrieveValue(srcdoc, "//wzsk:" + blk + "/wzsk:Fi[@num='" + num + "']/wzsk:tit2"));
		else if (i == 3) return(retrieveValue(srcdoc, "//wzsk:" + blk + "/wzsk:Fi[@num='" + num + "']/wzsk:tit3"));
	};
	return(retrieveValue(srcdoc, "//wzsk:" + blk + "/wzsk:Fi[@num='" + num + "']/wzsk:tit1"));
};
...
function retrieveFiSub(srcdoc, finode, sref) {
	...
	else res = srcdoc.evaluate("wzsk:" + sref, finode, nsResolver, XPathResult.ANY_TYPE, null);
...
function getRownode(srcdoc, blk, jnum) {
	return(getNode(srcdoc, "//wzsk:" + blk + "/wzsk:row[@jnum='" + jnum + "']"));
};

function retrieveRowField(srcdoc, rownode, sref) {
	var res, resnode;

	res = srcdoc.evaluate("wzsk:" + sref, rownode, nsResolver, XPathResult.ANY_TYPE, null);
	resnode = res.iterateNext();
