/**
  * \file PnlWzskOgrList.js
  * web client functionality for panel PnlWzskOgrList
...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlWzskOgrList_aside.html";
	doc.getElementById("List_cont").src = "./PnlWzskOgrList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlWzskOgrList_bside.html";
	doc.getElementById("List_cont").src = "./PnlWzskOgrList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWzskOgrList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWzskOgrList", "Trs"));
...
function init() {
	var srefIxWzskVExpstate = retrieveSi(srcdoc, "StatShrWzskOgrList", "srefIxWzskVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWzskOgrList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWzskOgrList", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWzskOgrList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzskOgrList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzskOgrList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWzskOgrList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWzskOgrList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWzskOgrList", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWzskOgrList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWzskOgrList", "ncol"));

		var TcoSrfWidth = parseInt(retrieveSi(srcdoc, "StgIacWzskOgrList", "TcoSrfWidth"));
		var TcoTitWidth = parseInt(retrieveSi(srcdoc, "StgIacWzskOgrList", "TcoTitWidth"));
		var TcoSupWidth = parseInt(retrieveSi(srcdoc, "StgIacWzskOgrList", "TcoSupWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacWzskOgrList", "numFTos"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagWzskOgrList", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWzskOgrList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWzskQOgrList
		rownode = getRownode(srcdoc, "ListWzskQOgrList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWzskOgrList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWzskOgrList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWzskOgrList", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WzskOgrList", "ListWzskQOgrList", retrieveTi(srcdoc, "TagWzskOgrList", "TxtRecord1"), retrieveTi(srcdoc, "TagWzskOgrList", "TxtRecord2"),
				["Srf","Tit","Sup"],
				["srf","tit","sup"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWzskOgrList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzskOgrList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWzskOgrList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWzskOgrList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWzskOgrList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWzskOgrListData", scrJref, "StgIacQryWzskOgrList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWzskOgrList", "firstcol"));

	setSi(srcdoc, "StatAppQryWzskOgrList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWzskOgrList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWzskOgrListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacWzskOgrList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacWzskOgrList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppWzskOgrListData", scrJref, "ContIacWzskOgrList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWzskOgrListData", "ContIacWzskOgrList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngWzskOgrListData", "ContInfWzskOgrList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWzskOgrListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWzskOgrListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngWzskOgrListData", "StatShrWzskOgrList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWzskOgrListData", "StgIacWzskOgrList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWzskOgrListData", "TagWzskOgrList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWzskOgrListData", "ListWzskQOgrList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWzskOgrListData", "StatAppQryWzskOgrList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWzskOgrListData", "StatShrQryWzskOgrList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWzskOgrListData", "StgIacQryWzskOgrList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngWzskOgrListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWzskOgrListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWzskOgrListData") {
				mask = mergeDpchEngData(dom);
