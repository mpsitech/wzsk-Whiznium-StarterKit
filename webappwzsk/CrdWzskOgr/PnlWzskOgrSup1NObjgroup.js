/**
  * \file PnlWzskOgrSup1NObjgroup.js
  * web client functionality for panel PnlWzskOgrSup1NObjgroup
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWzskOgrSup1NObjgroup", "srefIxWzskVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWzskOgrSup1NObjgroup", "srefIxWzskVExpstate", "mind");

	getRecwnd().changeHeight("Sup1NObjgroup", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("Sup1NObjgroup_side").setAttribute("height", "30");
	doc.getElementById("Sup1NObjgroup_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("Sup1NObjgroup_side").src = "./PnlWzskOgrSup1NObjgroup_aside.html";
	doc.getElementById("Sup1NObjgroup_cont").src = "./PnlWzskOgrSup1NObjgroup_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWzskOgrSup1NObjgroup", "srefIxWzskVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWzskOgrSup1NObjgroup", "srefIxWzskVExpstate", "regd");

	getRecwnd().changeHeight("Sup1NObjgroup", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("Sup1NObjgroup_side").setAttribute("height", "255");
	doc.getElementById("Sup1NObjgroup_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("Sup1NObjgroup_side").src = "./PnlWzskOgrSup1NObjgroup_bside.html";
	doc.getElementById("Sup1NObjgroup_cont").src = "./PnlWzskOgrSup1NObjgroup_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("Sup1NObjgroup_side").contentDocument;
	contdoc = doc.getElementById("Sup1NObjgroup_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("Sup1NObjgroup_side").contentDocument;
	contdoc = doc.getElementById("Sup1NObjgroup_cont").contentDocument;
	hdrdoc = contdoc.getElementById("Sup1NObjgroup_hdr").contentDocument;
	tbldoc = contdoc.getElementById("Sup1NObjgroup_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("Sup1NObjgroup_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWzskOgrSup1NObjgroup", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWzskOgrSup1NObjgroup", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWzskVExpstate = retrieveSi(srcdoc, "StatAppWzskOgrSup1NObjgroup", "srefIxWzskVExpstate");

	if (srefIxWzskVExpstate == "mind") initA();
	else if (srefIxWzskVExpstate == "regd") initBD(true);
	else if (srefIxWzskVExpstate == "detd") initBD(false);
};

function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWzskOgrSup1NObjgroup", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWzskOgrSup1NObjgroup", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWzskOgrSup1NObjgroup", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzskOgrSup1NObjgroup", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzskOgrSup1NObjgroup", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWzskOgrSup1NObjgroup", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWzskOgrSup1NObjgroup", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWzskOgrSup1NObjgroup", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWzskOgrSup1NObjgroup", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWzskOgrSup1NObjgroup", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWzskOgrSup1NObjgroup", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWzskOgrSup1NObjgroup", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWzskOgrSup1NObjgroup", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWzskQOgrSup1NObjgroup
		rownode = getRownode(srcdoc, "ListWzskQOgrSup1NObjgroup", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWzskOgrSup1NObjgroup", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWzskOgrSup1NObjgroup", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWzskOgrSup1NObjgroup", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("Sup1NObjgroup", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("Sup1NObjgroup_side").setAttribute("height", "" + (height+55));
	doc.getElementById("Sup1NObjgroup_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("Sup1NObjgroup_tbl").setAttribute("height", "" + height);
};

function refresh() {
	var srefIxWzskVExpstate = retrieveSi(srcdoc, "StatAppWzskOgrSup1NObjgroup", "srefIxWzskVExpstate");

	if (srefIxWzskVExpstate == "mind") refreshA();
	else if (srefIxWzskVExpstate == "regd") refreshBD(true, false);
	else if (srefIxWzskVExpstate == "detd") refreshBD(false, false);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("Sup1NObjgroup_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WzskOgrSup1NObjgroup", "ListWzskQOgrSup1NObjgroup", retrieveTi(srcdoc, "TagWzskOgrSup1NObjgroup", "TxtRecord1"), retrieveTi(srcdoc, "TagWzskOgrSup1NObjgroup", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWzskOgrSup1NObjgroup", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzskOgrSup1NObjgroup", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWzskOgrSup1NObjgroup", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWzskOgrSup1NObjgroup", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWzskOgrSup1NObjgroup", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWzskOgrSup1NObjgroupData", scrJref, "StgIacQryWzskOgrSup1NObjgroup");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWzskOgrSup1NObjgroup", "srefIxWzskVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWzskOgrSup1NObjgroup", "firstcol"));

	setSi(srcdoc, "StatAppQryWzskOgrSup1NObjgroup", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWzskOgrSup1NObjgroup", "srefIxWzskVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWzskOgrSup1NObjgroup", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWzskOgrSup1NObjgroupDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWzskOgrSup1NObjgroupData", "ContInfWzskOgrSup1NObjgroup", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWzskOgrSup1NObjgroupData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWzskOgrSup1NObjgroupData", "StatAppWzskOgrSup1NObjgroup", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWzskOgrSup1NObjgroupData", "StatShrWzskOgrSup1NObjgroup", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWzskOgrSup1NObjgroupData", "StgIacWzskOgrSup1NObjgroup", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWzskOgrSup1NObjgroupData", "TagWzskOgrSup1NObjgroup", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWzskOgrSup1NObjgroupData", "ListWzskQOgrSup1NObjgroup", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWzskOgrSup1NObjgroupData", "StatAppQryWzskOgrSup1NObjgroup", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWzskOgrSup1NObjgroupData", "StatShrQryWzskOgrSup1NObjgroup", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWzskOgrSup1NObjgroupData", "StgIacQryWzskOgrSup1NObjgroup", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWzskOgrSup1NObjgroupData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWzskOgrSup1NObjgroupData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("Sup1NObjgroup");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWzskOgrSup1NObjgroupData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
