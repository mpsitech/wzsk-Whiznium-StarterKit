/**
  * \file PnlWzskOgr1NShot.js
  * web client functionality for panel PnlWzskOgr1NShot
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWzskOgr1NShot", "srefIxWzskVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWzskOgr1NShot", "srefIxWzskVExpstate", "mind");
	...
	// change content
	doc.getElementById("1NShot_side").src = "./PnlWzskOgr1NShot_aside.html";
	doc.getElementById("1NShot_cont").src = "./PnlWzskOgr1NShot_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWzskOgr1NShot", "srefIxWzskVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWzskOgr1NShot", "srefIxWzskVExpstate", "regd");
	...
	// change content
	doc.getElementById("1NShot_side").src = "./PnlWzskOgr1NShot_bside.html";
	doc.getElementById("1NShot_cont").src = "./PnlWzskOgr1NShot_b.html";
};
...
function initBD(bNotD) {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWzskOgr1NShot", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWzskOgr1NShot", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWzskVExpstate = retrieveSi(srcdoc, "StatAppWzskOgr1NShot", "srefIxWzskVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWzskOgr1NShot", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWzskOgr1NShot", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWzskOgr1NShot", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzskOgr1NShot", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzskOgr1NShot", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWzskOgr1NShot", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWzskOgr1NShot", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWzskOgr1NShot", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWzskOgr1NShot", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWzskOgr1NShot", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWzskOgr1NShot", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWzskOgr1NShot", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWzskOgr1NShot", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWzskQOgr1NShot
		rownode = getRownode(srcdoc, "ListWzskQOgr1NShot", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWzskOgr1NShot", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWzskOgr1NShot", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWzskOgr1NShot", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WzskOgr1NShot", "ListWzskQOgr1NShot", retrieveTi(srcdoc, "TagWzskOgr1NShot", "TxtRecord1"), retrieveTi(srcdoc, "TagWzskOgr1NShot", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWzskOgr1NShot", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzskOgr1NShot", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWzskOgr1NShot", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWzskOgr1NShot", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWzskOgr1NShot", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWzskOgr1NShotData", scrJref, "StgIacQryWzskOgr1NShot");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWzskOgr1NShot", "srefIxWzskVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWzskOgr1NShot", "firstcol"));

	setSi(srcdoc, "StatAppQryWzskOgr1NShot", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWzskOgr1NShot", "srefIxWzskVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWzskOgr1NShot", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWzskOgr1NShotDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWzskOgr1NShotData", "ContInfWzskOgr1NShot", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWzskOgr1NShotData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWzskOgr1NShotData", "StatAppWzskOgr1NShot", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWzskOgr1NShotData", "StatShrWzskOgr1NShot", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWzskOgr1NShotData", "StgIacWzskOgr1NShot", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWzskOgr1NShotData", "TagWzskOgr1NShot", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWzskOgr1NShotData", "ListWzskQOgr1NShot", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWzskOgr1NShotData", "StatAppQryWzskOgr1NShot", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWzskOgr1NShotData", "StatShrQryWzskOgr1NShot", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWzskOgr1NShotData", "StgIacQryWzskOgr1NShot", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWzskOgr1NShotData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWzskOgr1NShotData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWzskOgr1NShotData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
