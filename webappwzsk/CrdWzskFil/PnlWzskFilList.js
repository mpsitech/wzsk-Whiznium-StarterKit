/**
  * \file PnlWzskFilList.js
  * web client functionality for panel PnlWzskFilList
...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlWzskFilList_aside.html";
	doc.getElementById("List_cont").src = "./PnlWzskFilList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlWzskFilList_bside.html";
	doc.getElementById("List_cont").src = "./PnlWzskFilList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWzskFilList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWzskFilList", "Trs"));
...
function init() {
	var srefIxWzskVExpstate = retrieveSi(srcdoc, "StatShrWzskFilList", "srefIxWzskVExpstate");
...
function refreshA() {
	var TxtFor = retrieveCi(srcdoc, "ContInfWzskFilList", "TxtFor");

	setTextContent(contdoc, contdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagWzskFilList", "TxtFor"), TxtFor));

	if (TxtFor == "") {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWzskFilList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWzskFilList", "ntot") + ")");
		clearElem(contdoc.getElementById("TxtPre"));
	} else {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWzskFilList", "Cpt"));
		setTextContent(contdoc, contdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfWzskFilList", "TxtPre") + " (" + retrieveSi(srcdoc, "StatShrQryWzskFilList", "ntot") + ")");
	};
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWzskFilList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzskFilList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzskFilList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWzskFilList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWzskFilList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWzskFilList", "ntot"));
	...
	// header row
	setTextContent(hdrdoc, hdrdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagWzskFilList", "TxtFor"), TxtFor));

	if (TxtFor == "") clearElem(hdrdoc.getElementById("TxtPre"));
	else setTextContent(hdrdoc, hdrdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfWzskFilList", "TxtPre"));

	// update column characteristics
	if (chgcol) {
		// obtain column (header) characteristics
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWzskFilList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWzskFilList", "ncol"));

		var TcoGrpWidth = parseInt(retrieveSi(srcdoc, "StgIacWzskFilList", "TcoGrpWidth"));
		var TcoOwnWidth = parseInt(retrieveSi(srcdoc, "StgIacWzskFilList", "TcoOwnWidth"));
		var TcoFnmWidth = parseInt(retrieveSi(srcdoc, "StgIacWzskFilList", "TcoFnmWidth"));
		var TcoRetWidth = parseInt(retrieveSi(srcdoc, "StgIacWzskFilList", "TcoRetWidth"));
		var TcoReuWidth = parseInt(retrieveSi(srcdoc, "StgIacWzskFilList", "TcoReuWidth"));
		var TcoCntWidth = parseInt(retrieveSi(srcdoc, "StgIacWzskFilList", "TcoCntWidth"));
		var TcoMimWidth = parseInt(retrieveSi(srcdoc, "StgIacWzskFilList", "TcoMimWidth"));
		var TcoSizWidth = parseInt(retrieveSi(srcdoc, "StgIacWzskFilList", "TcoSizWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacWzskFilList", "numFTos"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagWzskFilList", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWzskFilList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWzskQFilList
		rownode = getRownode(srcdoc, "ListWzskQFilList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWzskFilList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWzskFilList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWzskFilList", "numFCsiQst"));
...
function refresh(chgcol) {
	...
	else if (srefIxWzskVExpstate == "regd") refreshB(chgcol);
};
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WzskFilList", "ListWzskQFilList", retrieveTi(srcdoc, "TagWzskFilList", "TxtRecord1"), retrieveTi(srcdoc, "TagWzskFilList", "TxtRecord2"),
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWzskFilList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzskFilList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWzskFilList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWzskFilList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWzskFilList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWzskFilListData", scrJref, "StgIacQryWzskFilList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWzskFilList", "firstcol"));

	setSi(srcdoc, "StatAppQryWzskFilList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWzskFilList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWzskFilListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacWzskFilList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacWzskFilList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppWzskFilListData", scrJref, "ContIacWzskFilList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWzskFilListData", "ContIacWzskFilList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngWzskFilListData", "ContInfWzskFilList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWzskFilListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWzskFilListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngWzskFilListData", "StatShrWzskFilList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWzskFilListData", "StgIacWzskFilList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWzskFilListData", "TagWzskFilList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWzskFilListData", "ListWzskQFilList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWzskFilListData", "StatAppQryWzskFilList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWzskFilListData", "StatShrQryWzskFilList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWzskFilListData", "StgIacQryWzskFilList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngWzskFilListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWzskFilListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWzskFilListData") {
				mask = mergeDpchEngData(dom);
