...
function minimize(updh) {
	...
	setSi(srcdoc, "StatAppWzskObj1NShot", "srefIxWzskVExpstate", "mind");
	...
	// change content
	doc.getElementById("1NShot_side").src = "./PnlWzskObj1NShot_aside.html";
	doc.getElementById("1NShot_cont").src = "./PnlWzskObj1NShot_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWzskObj1NShot", "srefIxWzskVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWzskObj1NShot", "srefIxWzskVExpstate", "regd");
	...
	// change content
	doc.getElementById("1NShot_side").src = "./PnlWzskObj1NShot_bside.html";
	doc.getElementById("1NShot_cont").src = "./PnlWzskObj1NShot_b.html";
};
...
function initBD(bNotD) {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWzskObj1NShot", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWzskObj1NShot", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWzskVExpstate = retrieveSi(srcdoc, "StatAppWzskObj1NShot", "srefIxWzskVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWzskObj1NShot", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWzskObj1NShot", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWzskObj1NShot", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzskObj1NShot", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzskObj1NShot", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWzskObj1NShot", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWzskObj1NShot", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWzskObj1NShot", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWzskObj1NShot", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWzskObj1NShot", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWzskObj1NShot", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWzskObj1NShot", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWzskObj1NShot", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWzskQObj1NShot
		rownode = getRownode(srcdoc, "ListWzskQObj1NShot", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWzskObj1NShot", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWzskObj1NShot", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWzskObj1NShot", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WzskObj1NShot", "ListWzskQObj1NShot", retrieveTi(srcdoc, "TagWzskObj1NShot", "TxtRecord1"), retrieveTi(srcdoc, "TagWzskObj1NShot", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWzskObj1NShot", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzskObj1NShot", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWzskObj1NShot", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWzskObj1NShot", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWzskObj1NShot", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWzskObj1NShotData", scrJref, "StgIacQryWzskObj1NShot");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWzskObj1NShot", "srefIxWzskVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWzskObj1NShot", "firstcol"));

	setSi(srcdoc, "StatAppQryWzskObj1NShot", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWzskObj1NShot", "srefIxWzskVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWzskObj1NShot", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWzskObj1NShotDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWzskObj1NShotData", "ContInfWzskObj1NShot", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWzskObj1NShotData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWzskObj1NShotData", "StatAppWzskObj1NShot", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWzskObj1NShotData", "StatShrWzskObj1NShot", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWzskObj1NShotData", "StgIacWzskObj1NShot", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWzskObj1NShotData", "TagWzskObj1NShot", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWzskObj1NShotData", "ListWzskQObj1NShot", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWzskObj1NShotData", "StatAppQryWzskObj1NShot", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWzskObj1NShotData", "StatShrQryWzskObj1NShot", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWzskObj1NShotData", "StgIacQryWzskObj1NShot", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWzskObj1NShotData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWzskObj1NShotData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWzskObj1NShotData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
