...
function getInitdone(pnlshort) {
	return(retrieveSi(srcdoc, "StatAppWzskObjRec", "initdone" + pnlshort) == "true");
};

function setInitdone(pnlshort) {
	setSi(srcdoc, "StatAppWzskObjRec", "initdone" + pnlshort, "true");
...
function checkInitdone() {
	...
	var initdoneAPar = (retrieveSi(srcdoc, "StatAppWzskObjRec", "initdoneAPar") == "true");
...
function minimize() {
	...
	doc.getElementById("Rec_side").src = "./PnlWzskObjRec_aside.html";
	doc.getElementById("Rec_cont").src = "./PnlWzskObjRec_a.html";
};
...
function initA() {
	...
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWzskObjRec", "Cpt"));
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWzskObjRec", "Cpt"));

	checkInitdone();
};

function init() {
	var srefIxWzskVExpstate = retrieveSi(srcdoc, "StatShrWzskObjRec", "srefIxWzskVExpstate");
...
function refreshAside() {
	...
	ButRegularizeActive = (retrieveSi(srcdoc, "StatShrWzskObjRec", "ButRegularizeActive") == "true");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("TxtRef"), retrieveCi(srcdoc, "ContInfWzskObjRec", "TxtRef"));
};

function refreshB() {

	setTextContent(hdrdoc, hdrdoc.getElementById("TxtRef"), retrieveCi(srcdoc, "ContInfWzskObjRec", "TxtRef"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWzskObjRecDo", scrJref, ctlsref + "Click");
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWzskObjRecData", "ContInfWzskObjRec", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWzskObjRecData", "StatAppWzskObjRec", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWzskObjRecData", "StatShrWzskObjRec", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWzskObjRecData", "TagWzskObjRec", srcdoc)) mask.push("tag");
...
function handleDpchEng(dom, dpch) {
	...
	srefIxWzskVExpstate = retrieveSi(srcdoc, "StatShrWzskObjRec", "srefIxWzskVExpstate");

	if (_scrJref == scrJref) {
		if (dpch == "DpchEngWzskObjRecData") {
			mask = mergeDpchEngData(dom);

			if (mask.indexOf("statshr") != -1) {
				newSrefIxWzskVExpstate = retrieveSi(srcdoc, "StatShrWzskObjRec", "srefIxWzskVExpstate");
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWzskObjRecData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWzskObjRecData") {
				srefIxWzskVExpstate = retrieveSi(srcdoc, "StatShrWzskObjRec", "srefIxWzskVExpstate");
