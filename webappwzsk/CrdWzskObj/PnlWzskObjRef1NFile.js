...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWzskObjRef1NFile", "srefIxWzskVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWzskObjRef1NFile", "srefIxWzskVExpstate", "mind");
	...
	// change content
	doc.getElementById("Ref1NFile_side").src = "./PnlWzskObjRef1NFile_aside.html";
	doc.getElementById("Ref1NFile_cont").src = "./PnlWzskObjRef1NFile_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWzskObjRef1NFile", "srefIxWzskVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWzskObjRef1NFile", "srefIxWzskVExpstate", "regd");
	...
	// change content
	doc.getElementById("Ref1NFile_side").src = "./PnlWzskObjRef1NFile_bside.html";
	doc.getElementById("Ref1NFile_cont").src = "./PnlWzskObjRef1NFile_b.html";
};
...
function initBD(bNotD) {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWzskObjRef1NFile", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWzskObjRef1NFile", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWzskVExpstate = retrieveSi(srcdoc, "StatAppWzskObjRef1NFile", "srefIxWzskVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWzskObjRef1NFile", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWzskObjRef1NFile", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWzskObjRef1NFile", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzskObjRef1NFile", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzskObjRef1NFile", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWzskObjRef1NFile", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWzskObjRef1NFile", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWzskObjRef1NFile", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWzskObjRef1NFile", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWzskObjRef1NFile", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWzskObjRef1NFile", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWzskObjRef1NFile", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWzskObjRef1NFile", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWzskQObjRef1NFile
		rownode = getRownode(srcdoc, "ListWzskQObjRef1NFile", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWzskObjRef1NFile", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWzskObjRef1NFile", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWzskObjRef1NFile", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WzskObjRef1NFile", "ListWzskQObjRef1NFile", retrieveTi(srcdoc, "TagWzskObjRef1NFile", "TxtRecord1"), retrieveTi(srcdoc, "TagWzskObjRef1NFile", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWzskObjRef1NFile", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzskObjRef1NFile", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWzskObjRef1NFile", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWzskObjRef1NFile", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWzskObjRef1NFile", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWzskObjRef1NFileData", scrJref, "StgIacQryWzskObjRef1NFile");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWzskObjRef1NFile", "srefIxWzskVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWzskObjRef1NFile", "firstcol"));

	setSi(srcdoc, "StatAppQryWzskObjRef1NFile", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWzskObjRef1NFile", "srefIxWzskVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWzskObjRef1NFile", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWzskObjRef1NFileDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWzskObjRef1NFileData", "ContInfWzskObjRef1NFile", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWzskObjRef1NFileData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWzskObjRef1NFileData", "StatAppWzskObjRef1NFile", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWzskObjRef1NFileData", "StatShrWzskObjRef1NFile", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWzskObjRef1NFileData", "StgIacWzskObjRef1NFile", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWzskObjRef1NFileData", "TagWzskObjRef1NFile", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWzskObjRef1NFileData", "ListWzskQObjRef1NFile", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWzskObjRef1NFileData", "StatAppQryWzskObjRef1NFile", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWzskObjRef1NFileData", "StatShrQryWzskObjRef1NFile", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWzskObjRef1NFileData", "StgIacQryWzskObjRef1NFile", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWzskObjRef1NFileData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWzskObjRef1NFileData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWzskObjRef1NFileData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
