function getInitdone(pnlshort) {
	return(retrieveSi(srcdoc, "StatAppWzskObj", "initdone" + pnlshort) == "true");
};

function setInitdone(pnlshort) {
	setSi(srcdoc, "StatAppWzskObj", "initdone" + pnlshort, "true");

	checkInitdone();
};

function checkInitdone() {
	var initdoneHeadbar = (retrieveSi(srcdoc, "StatAppWzskObj", "initdoneHeadbar") == "true");
	var initdoneList = (retrieveSi(srcdoc, "StatAppWzskObj", "initdoneList") == "true");
	var initdoneRec = (retrieveSi(srcdoc, "StatAppWzskObj", "initdoneRec") == "true");

	if (!initdoneHeadbar) {
		doc.getElementById("Headbar").src = "./PnlWzskObjHeadbar.html?scrJref=" + scrJrefHeadbar;
	} else if (!initdoneList) {
		doc.getElementById("List").src = "./PnlWzskObjList.html?scrJref=" + scrJrefList;
	} else if (!initdoneRec) {
		doc.getElementById("Rec").src = "./PnlWzskObjRec.html?scrJref=" + scrJrefRec;
	} else {
		doc.initdone = true;
...
function toggleMenu(short, left, width, height) {
	var shortMenu = retrieveSi(srcdoc, "StatAppWzskObj", "shortMenu");
...
function showMenu(short, left, width, height) {
	...
	setSi(srcdoc, "StatAppWzskObj", "shortMenu", short);
	setSi(srcdoc, "StatAppWzskObj", "widthMenu", "" + (width-3));
...
function hideMenu(short) {
	...
	setSi(srcdoc, "StatAppWzskObj", "shortMenu", "");
...
function initMenApp() {
	...
	mendoc.getElementById("colCont").setAttribute("width", retrieveSi(srcdoc, "StatAppWzskObj", "widthMenu"));

	setTextContent(mendoc, mendoc.getElementById("MitAppAbt"), retrieveTi(srcdoc, "TagWzskObj", "MitAppAbt"));
	setTextContent(mendoc, mendoc.getElementById("MrlAppHlp"), retrieveTi(srcdoc, "TagWzskObj", "MrlAppHlp"));
};
...
function initMenCrd() {
	...
	MspCrd1Avail = (retrieveSi(srcdoc, "StatShrWzskObj", "MspCrd1Avail") == "true");
	MitCrdNewAvail = (retrieveSi(srcdoc, "StatShrWzskObj", "MitCrdNewAvail") == "true");

	mendoc.getElementById("colCont").setAttribute("width", retrieveSi(srcdoc, "StatAppWzskObj", "widthMenu"));

	setTextContent(mendoc, mendoc.getElementById("MtxCrdObj"), retrieveCi(srcdoc, "ContInfWzskObj", "MtxCrdObj"));
	setTextContent(mendoc, mendoc.getElementById("MitCrdNew"), retrieveTi(srcdoc, "TagWzskObj", "MitCrdNew"));
...
function refresh() {
	var scrJrefDlgnew = retrieveSi(srcdoc, "StatShrWzskObj", "scrJrefDlgnew");

	if (scrJrefDlgnew != "") {
		if (scrJrefDlg != scrJrefDlgnew) showDlg("DlgWzskObjNew", scrJrefDlgnew);
	} else if (scrJrefDlg != "") hideDlg();

	doc.title = retrieveCi(srcdoc, "ContInfWzskObj", "MtxCrdObj") + " - Whiznium StarterKit v1.0.12";
};
...
function handleUnload() {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWzskObjDo", scrJref, "close");
	sendReq(str, doc, null, true);
};

function handleMitClick(menshort, consref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWzskObjDo", scrJref, consref + "Click");
	sendReq(str, doc, handleDpchAppDoReply);
...
function handleMrlClick(menshort, consref) {
	window.open(retrieveCi(srcdoc, "ContInfWzskObj", consref), "_blank");
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWzskObjData", "ContInfWzskObj", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWzskObjData", "FeedFSge", srcdoc)) mask.push("feedFSge");
	if (updateSrcblock(dom, "DpchEngWzskObjData", "StatAppWzskObj", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWzskObjData", "StatShrWzskObj", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWzskObjData", "TagWzskObj", srcdoc)) mask.push("tag");
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWzskObjData") {
				mergeDpchEngData(dom);

				// establish sub-panel scrJref's as global variables, and start sub-panel load sequence
				scrJrefHeadbar = retrieveSi(srcdoc, "StatShrWzskObj", "scrJrefHeadbar");
				scrJrefList = retrieveSi(srcdoc, "StatShrWzskObj", "scrJrefList");
				scrJrefRec = retrieveSi(srcdoc, "StatShrWzskObj", "scrJrefRec");
		...
		if (blk) {
			...
			if (_scrJref != "") {
				if (_scrJref == scrJref) {
					...
					} else if (blk.nodeName == "DpchEngWzskObjData") {
						mergeDpchEngData(dom);
						refresh();
					};
...
function restartNotify() {
	var srefIxWzskVReqitmode = retrieveSi(srcdoc, "StatAppWzskObj", "srefIxWzskVReqitmode");

	if (srefIxWzskVReqitmode != "notify") {
		if (doc.timeout) window.clearTimeout(doc.timeout);

		srefIxWzskVReqitmode = "notify";
		setSi(srcdoc, "StatAppWzskObj", "srefIxWzskVReqitmode", srefIxWzskVReqitmode);
...
function iterateReqit(ev) {
	...
	var srefIxWzskVReqitmode = retrieveSi(srcdoc, "StatAppWzskObj", "srefIxWzskVReqitmode");
	var latency = parseInt(retrieveSi(srcdoc, "StatAppWzskObj", "latency"));
