/**
  * \file PnlWzskObjList.js
  * web client functionality for panel PnlWzskObjList
...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlWzskObjList_aside.html";
	doc.getElementById("List_cont").src = "./PnlWzskObjList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlWzskObjList_bside.html";
	doc.getElementById("List_cont").src = "./PnlWzskObjList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWzskObjList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWzskObjList", "Trs"));
...
function init() {
	var srefIxWzskVExpstate = retrieveSi(srcdoc, "StatShrWzskObjList", "srefIxWzskVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWzskObjList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWzskObjList", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWzskObjList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzskObjList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzskObjList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWzskObjList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWzskObjList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWzskObjList", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWzskObjList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWzskObjList", "ncol"));

		var TcoTitWidth = parseInt(retrieveSi(srcdoc, "StgIacWzskObjList", "TcoTitWidth"));
		var TcoOgrWidth = parseInt(retrieveSi(srcdoc, "StgIacWzskObjList", "TcoOgrWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacWzskObjList", "numFTos"));
		...
		for (i = firstcol, ix = 0; i <= ncol; i++) {
			...
			if (i == 1) {
				...
				doc.cols.push("TcoOgr"); doc.widths.push(TcoOgrWidth); doc.fs.push("ogr"); doc.tos.push(1); wtot += TcoOgrWidth;
			} else {
				ix--;
			};
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagWzskObjList", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWzskObjList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWzskQObjList
		rownode = getRownode(srcdoc, "ListWzskQObjList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWzskObjList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWzskObjList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWzskObjList", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WzskObjList", "ListWzskQObjList", retrieveTi(srcdoc, "TagWzskObjList", "TxtRecord1"), retrieveTi(srcdoc, "TagWzskObjList", "TxtRecord2"),
				["Tit","Ogr"],
				["tit","ogr"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWzskObjList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzskObjList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWzskObjList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWzskObjList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWzskObjList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWzskObjListData", scrJref, "StgIacQryWzskObjList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWzskObjList", "firstcol"));

	setSi(srcdoc, "StatAppQryWzskObjList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWzskObjList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWzskObjListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacWzskObjList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacWzskObjList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppWzskObjListData", scrJref, "ContIacWzskObjList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWzskObjListData", "ContIacWzskObjList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngWzskObjListData", "ContInfWzskObjList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWzskObjListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWzskObjListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngWzskObjListData", "StatShrWzskObjList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWzskObjListData", "StgIacWzskObjList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWzskObjListData", "TagWzskObjList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWzskObjListData", "ListWzskQObjList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWzskObjListData", "StatAppQryWzskObjList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWzskObjListData", "StatShrQryWzskObjList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWzskObjListData", "StgIacQryWzskObjList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngWzskObjListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWzskObjListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWzskObjListData") {
				mask = mergeDpchEngData(dom);
