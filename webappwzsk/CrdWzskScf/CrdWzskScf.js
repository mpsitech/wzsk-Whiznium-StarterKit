...
function setInitdone(pnlshort) {
	setSi(srcdoc, "StatAppWzskScf", "initdone" + pnlshort, "true");

	checkInitdone();
};

function checkInitdone() {
	var initdoneConn = (retrieveSi(srcdoc, "StatAppWzskScf", "initdoneConn") == "true");
	var initdoneGeom = (retrieveSi(srcdoc, "StatAppWzskScf", "initdoneGeom") == "true");
	var initdoneHeadbar = (retrieveSi(srcdoc, "StatAppWzskScf", "initdoneHeadbar") == "true");

	if (!initdoneConn) {
		doc.getElementById("Conn").src = "./PnlWzskScfConn.html?scrJref=" + scrJrefConn;
	} else if (!initdoneGeom) {
		doc.getElementById("Geom").src = "./PnlWzskScfGeom.html?scrJref=" + scrJrefGeom;
	} else if (!initdoneHeadbar) {
		doc.getElementById("Headbar").src = "./PnlWzskScfHeadbar.html?scrJref=" + scrJrefHeadbar;
	} else {
		doc.initdone = true;
...
function getHeight() {
	...
	height += 10 + parseInt(doc.getElementById("tdGeom").getAttribute("height")) + 8;
...
function toggleMenu(short, left, width, height) {
	var shortMenu = retrieveSi(srcdoc, "StatAppWzskScf", "shortMenu");
...
function showMenu(short, left, width, height) {
	...
	setSi(srcdoc, "StatAppWzskScf", "shortMenu", short);
	setSi(srcdoc, "StatAppWzskScf", "widthMenu", "" + (width-3));
...
function hideMenu(short) {
	...
	setSi(srcdoc, "StatAppWzskScf", "shortMenu", "");
...
function initMenApp() {
	...
	mendoc.getElementById("colCont").setAttribute("width", retrieveSi(srcdoc, "StatAppWzskScf", "widthMenu"));

	setTextContent(mendoc, mendoc.getElementById("MitAppAbt"), retrieveTi(srcdoc, "TagWzskScf", "MitAppAbt"));
	setTextContent(mendoc, mendoc.getElementById("MrlAppHlp"), retrieveTi(srcdoc, "TagWzskScf", "MrlAppHlp"));
};
...
function initMenCrd() {
	...
	setTextContent(mendoc, mendoc.getElementById("MitCrdCmm"), retrieveTi(srcdoc, "TagWzskScf", "MitCrdCmm"));
	setTextContent(mendoc, mendoc.getElementById("MitCrdTtc"), retrieveTi(srcdoc, "TagWzskScf", "MitCrdTtc"));
	setTextContent(mendoc, mendoc.getElementById("MitCrdLsp"), retrieveTi(srcdoc, "TagWzskScf", "MitCrdLsp"));
};
...
function showDlg(sref, _scrJref)  {
	...
	if ((sref == "DlgWzskScfCameramat") || (sref == "DlgWzskScfTtablecoord") || (sref == "DlgWzskScfLaserpos")) myif.setAttribute("height", "585");
...
function changeHeight(pnlshort, height) {
	...
	} else if (pnlshort == "Geom") {
		doc.getElementById("tdGeom").setAttribute("height", "" + height);
		doc.getElementById("Geom").setAttribute("height", "" + height);
	};
...
function refresh() {
	var scrJrefDlgcameramat = retrieveSi(srcdoc, "StatShrWzskScf", "scrJrefDlgcameramat");
	var scrJrefDlglaserpos = retrieveSi(srcdoc, "StatShrWzskScf", "scrJrefDlglaserpos");
	var scrJrefDlgttablecoord = retrieveSi(srcdoc, "StatShrWzskScf", "scrJrefDlgttablecoord");

	if (scrJrefDlgcameramat != "") {
		if (scrJrefDlg != scrJrefDlgcameramat) showDlg("DlgWzskScfCameramat", scrJrefDlgcameramat);
	} else if (scrJrefDlglaserpos != "") {
		if (scrJrefDlg != scrJrefDlglaserpos) showDlg("DlgWzskScfLaserpos", scrJrefDlglaserpos);
	} else if (scrJrefDlgttablecoord != "") {
		if (scrJrefDlg != scrJrefDlgttablecoord) showDlg("DlgWzskScfTtablecoord", scrJrefDlgttablecoord);
...
function handleUnload() {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWzskScfDo", scrJref, "close");
	sendReq(str, doc, null, true);
};

function handleMitClick(menshort, consref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWzskScfDo", scrJref, consref + "Click");
	sendReq(str, doc, handleDpchAppDoReply);
...
function handleMrlClick(menshort, consref) {
	window.open(retrieveCi(srcdoc, "ContInfWzskScf", consref), "_blank");
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWzskScfData", "ContInfWzskScf", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWzskScfData", "FeedFSge", srcdoc)) mask.push("feedFSge");
	if (updateSrcblock(dom, "DpchEngWzskScfData", "StatAppWzskScf", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWzskScfData", "StatShrWzskScf", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWzskScfData", "TagWzskScf", srcdoc)) mask.push("tag");
...
function handleDpchEngSub(_scrJref, dom, dpch) {
	if (_scrJref == scrJrefPnld) {
		...
		doc.getElementById("Conn").contentWindow.handleDpchEng(dom, dpch);
	} else if (_scrJref == scrJrefGeom) {
		doc.getElementById("Geom").contentWindow.handleDpchEng(dom, dpch);
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWzskScfData") {
				mergeDpchEngData(dom);

				// establish sub-panel scrJref's as global variables, and start sub-panel load sequence
				scrJrefConn = retrieveSi(srcdoc, "StatShrWzskScf", "scrJrefConn");
				scrJrefGeom = retrieveSi(srcdoc, "StatShrWzskScf", "scrJrefGeom");
				scrJrefHeadbar = retrieveSi(srcdoc, "StatShrWzskScf", "scrJrefHeadbar");
		...
		if (blk) {
			...
			if (_scrJref != "") {
				if (_scrJref == scrJref) {
					...
					} else if (blk.nodeName == "DpchEngWzskScfData") {
						mergeDpchEngData(dom);
						refresh();
					};
...
function restartNotify() {
	var srefIxWzskVReqitmode = retrieveSi(srcdoc, "StatAppWzskScf", "srefIxWzskVReqitmode");

	if (srefIxWzskVReqitmode != "notify") {
		if (doc.timeout) window.clearTimeout(doc.timeout);

		srefIxWzskVReqitmode = "notify";
		setSi(srcdoc, "StatAppWzskScf", "srefIxWzskVReqitmode", srefIxWzskVReqitmode);
...
function iterateReqit(ev) {
	...
	var srefIxWzskVReqitmode = retrieveSi(srcdoc, "StatAppWzskScf", "srefIxWzskVReqitmode");
	var latency = parseInt(retrieveSi(srcdoc, "StatAppWzskScf", "latency"));
