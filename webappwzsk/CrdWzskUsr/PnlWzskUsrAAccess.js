/**
  * \file PnlWzskUsrAAccess.js
  * web client functionality for panel PnlWzskUsrAAccess
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWzskUsrAAccess", "srefIxWzskVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWzskUsrAAccess", "srefIxWzskVExpstate", "mind");
	...
	// change content
	doc.getElementById("AAccess_side").src = "./PnlWzskUsrAAccess_aside.html";
	doc.getElementById("AAccess_cont").src = "./PnlWzskUsrAAccess_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWzskUsrAAccess", "srefIxWzskVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWzskUsrAAccess", "srefIxWzskVExpstate", "regd");
	...
	// change content
	doc.getElementById("AAccess_side").src = "./PnlWzskUsrAAccess_bside.html";
	doc.getElementById("AAccess_cont").src = "./PnlWzskUsrAAccess_b.html";
};
...
function initBD(bNotD) {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWzskUsrAAccess", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWzskUsrAAccess", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWzskVExpstate = retrieveSi(srcdoc, "StatAppWzskUsrAAccess", "srefIxWzskVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWzskUsrAAccess", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWzskUsrAAccess", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWzskUsrAAccess", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzskUsrAAccess", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzskUsrAAccess", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWzskUsrAAccess", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWzskUsrAAccess", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWzskUsrAAccess", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWzskUsrAAccess", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWzskUsrAAccess", "ncol"));

		var TcoFegWidth = parseInt(retrieveSi(srcdoc, "StgIacWzskUsrAAccess", "TcoFegWidth"));
		var TcoFeaWidth = parseInt(retrieveSi(srcdoc, "StgIacWzskUsrAAccess", "TcoFeaWidth"));
		var TcoAccWidth = parseInt(retrieveSi(srcdoc, "StgIacWzskUsrAAccess", "TcoAccWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWzskUsrAAccess", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWzskUsrAAccess", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWzskQUsrAAccess
		rownode = getRownode(srcdoc, "ListWzskQUsrAAccess", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWzskUsrAAccess", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWzskUsrAAccess", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWzskUsrAAccess", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WzskUsrAAccess", "ListWzskQUsrAAccess", retrieveTi(srcdoc, "TagWzskUsrAAccess", "TxtRecord1"), retrieveTi(srcdoc, "TagWzskUsrAAccess", "TxtRecord2"),
				["Feg","","Fea","Acc",""],
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWzskUsrAAccess", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzskUsrAAccess", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWzskUsrAAccess", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWzskUsrAAccess", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWzskUsrAAccess", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWzskUsrAAccessData", scrJref, "StgIacQryWzskUsrAAccess");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWzskUsrAAccess", "srefIxWzskVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWzskUsrAAccess", "firstcol"));

	setSi(srcdoc, "StatAppQryWzskUsrAAccess", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWzskUsrAAccess", "srefIxWzskVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWzskUsrAAccess", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWzskUsrAAccessDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWzskUsrAAccessData", "ContInfWzskUsrAAccess", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWzskUsrAAccessData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWzskUsrAAccessData", "StatAppWzskUsrAAccess", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWzskUsrAAccessData", "StatShrWzskUsrAAccess", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWzskUsrAAccessData", "StgIacWzskUsrAAccess", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWzskUsrAAccessData", "TagWzskUsrAAccess", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWzskUsrAAccessData", "ListWzskQUsrAAccess", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWzskUsrAAccessData", "StatAppQryWzskUsrAAccess", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWzskUsrAAccessData", "StatShrQryWzskUsrAAccess", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWzskUsrAAccessData", "StgIacQryWzskUsrAAccess", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWzskUsrAAccessData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWzskUsrAAccessData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWzskUsrAAccessData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
