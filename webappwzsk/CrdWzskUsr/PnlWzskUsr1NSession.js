...
function minimize(updh) {
	...
	setSi(srcdoc, "StatAppWzskUsr1NSession", "srefIxWzskVExpstate", "mind");
	...
	// change content
	doc.getElementById("1NSession_side").src = "./PnlWzskUsr1NSession_aside.html";
	doc.getElementById("1NSession_cont").src = "./PnlWzskUsr1NSession_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWzskUsr1NSession", "srefIxWzskVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWzskUsr1NSession", "srefIxWzskVExpstate", "regd");
	...
	// change content
	doc.getElementById("1NSession_side").src = "./PnlWzskUsr1NSession_bside.html";
	doc.getElementById("1NSession_cont").src = "./PnlWzskUsr1NSession_b.html";
};
...
function initBD(bNotD) {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWzskUsr1NSession", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWzskUsr1NSession", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWzskVExpstate = retrieveSi(srcdoc, "StatAppWzskUsr1NSession", "srefIxWzskVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWzskUsr1NSession", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWzskUsr1NSession", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWzskUsr1NSession", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzskUsr1NSession", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzskUsr1NSession", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWzskUsr1NSession", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWzskUsr1NSession", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWzskUsr1NSession", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWzskUsr1NSession", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWzskUsr1NSession", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWzskUsr1NSession", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWzskUsr1NSession", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWzskUsr1NSession", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWzskQUsr1NSession
		rownode = getRownode(srcdoc, "ListWzskQUsr1NSession", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWzskUsr1NSession", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWzskUsr1NSession", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWzskUsr1NSession", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WzskUsr1NSession", "ListWzskQUsr1NSession", retrieveTi(srcdoc, "TagWzskUsr1NSession", "TxtRecord1"), retrieveTi(srcdoc, "TagWzskUsr1NSession", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWzskUsr1NSession", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzskUsr1NSession", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWzskUsr1NSession", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWzskUsr1NSession", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWzskUsr1NSession", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWzskUsr1NSessionData", scrJref, "StgIacQryWzskUsr1NSession");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWzskUsr1NSession", "srefIxWzskVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWzskUsr1NSession", "firstcol"));

	setSi(srcdoc, "StatAppQryWzskUsr1NSession", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWzskUsr1NSession", "srefIxWzskVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWzskUsr1NSession", "jnum"));
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWzskUsr1NSessionData", "ContInfWzskUsr1NSession", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWzskUsr1NSessionData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWzskUsr1NSessionData", "StatAppWzskUsr1NSession", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWzskUsr1NSessionData", "StatShrWzskUsr1NSession", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWzskUsr1NSessionData", "StgIacWzskUsr1NSession", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWzskUsr1NSessionData", "TagWzskUsr1NSession", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWzskUsr1NSessionData", "ListWzskQUsr1NSession", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWzskUsr1NSessionData", "StatAppQryWzskUsr1NSession", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWzskUsr1NSessionData", "StatShrQryWzskUsr1NSession", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWzskUsr1NSessionData", "StgIacQryWzskUsr1NSession", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWzskUsr1NSessionData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWzskUsr1NSessionData") {
				mergeDpchEngData(dom);
				init();
