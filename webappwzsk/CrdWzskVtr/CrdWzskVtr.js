function getInitdone(pnlshort) {
	return(retrieveSi(srcdoc, "StatAppWzskVtr", "initdone" + pnlshort) == "true");
};

function setInitdone(pnlshort) {
	setSi(srcdoc, "StatAppWzskVtr", "initdone" + pnlshort, "true");

	checkInitdone();
};

function checkInitdone() {
	var initdoneConfig = (retrieveSi(srcdoc, "StatAppWzskVtr", "initdoneConfig") == "true");
	var initdoneMonitor = (retrieveSi(srcdoc, "StatAppWzskVtr", "initdoneMonitor") == "true");
	var initdoneDebug = (retrieveSi(srcdoc, "StatAppWzskVtr", "initdoneDebug") == "true");
	var initdoneHeadbar = (retrieveSi(srcdoc, "StatAppWzskVtr", "initdoneHeadbar") == "true");

	if (!initdoneConfig) {
		doc.getElementById("Config").src = "./PnlWzskVtrConfig.html?scrJref=" + scrJrefConfig;
	} else if (!initdoneMonitor) {
		doc.getElementById("Monitor").src = "./PnlWzskVtrMonitor.html?scrJref=" + scrJrefMonitor;
	} else if (!initdoneDebug) {
		doc.getElementById("Debug").src = "./PnlWzskVtrDebug.html?scrJref=" + scrJrefDebug;
	} else if (!initdoneHeadbar) {
		doc.getElementById("Headbar").src = "./PnlWzskVtrHeadbar.html?scrJref=" + scrJrefHeadbar;
	} else {
		doc.initdone = true;
...
function getHeight() {
	...
	height += 10 + parseInt(doc.getElementById("tdConfig").getAttribute("height")) + 8;
	height += 10 + parseInt(doc.getElementById("tdMonitor").getAttribute("height")) + 8;
	height += 10 + parseInt(doc.getElementById("tdDebug").getAttribute("height")) + 8;
...
function toggleMenu(short, left, width, height) {
	var shortMenu = retrieveSi(srcdoc, "StatAppWzskVtr", "shortMenu");
...
function showMenu(short, left, width, height) {
	...
	setSi(srcdoc, "StatAppWzskVtr", "shortMenu", short);
	setSi(srcdoc, "StatAppWzskVtr", "widthMenu", "" + (width-3));
...
function hideMenu(short) {
	...
	setSi(srcdoc, "StatAppWzskVtr", "shortMenu", "");
...
function initMenApp() {
	...
	mendoc.getElementById("colCont").setAttribute("width", retrieveSi(srcdoc, "StatAppWzskVtr", "widthMenu"));

	setTextContent(mendoc, mendoc.getElementById("MitAppAbt"), retrieveTi(srcdoc, "TagWzskVtr", "MitAppAbt"));
	setTextContent(mendoc, mendoc.getElementById("MrlAppHlp"), retrieveTi(srcdoc, "TagWzskVtr", "MrlAppHlp"));
};
...
function changeHeight(pnlshort, height) {
	...
	if (pnlshort == "Config") {
		doc.getElementById("tdConfig").setAttribute("height", "" + height);
		doc.getElementById("Config").setAttribute("height", "" + height);
	} else if (pnlshort == "Monitor") {
		doc.getElementById("tdMonitor").setAttribute("height", "" + height);
		doc.getElementById("Monitor").setAttribute("height", "" + height);
	} else if (pnlshort == "Debug") {
		doc.getElementById("tdDebug").setAttribute("height", "" + height);
		doc.getElementById("Debug").setAttribute("height", "" + height);
	};
...
function handleUnload() {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWzskVtrDo", scrJref, "close");
	sendReq(str, doc, null, true);
};

function handleMitClick(menshort, consref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWzskVtrDo", scrJref, consref + "Click");
	sendReq(str, doc, handleDpchAppDoReply);
...
function handleMrlClick(menshort, consref) {
	window.open(retrieveCi(srcdoc, "ContInfWzskVtr", consref), "_blank");
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWzskVtrData", "ContInfWzskVtr", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWzskVtrData", "FeedFSge", srcdoc)) mask.push("feedFSge");
	if (updateSrcblock(dom, "DpchEngWzskVtrData", "StatAppWzskVtr", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWzskVtrData", "StatShrWzskVtr", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWzskVtrData", "TagWzskVtr", srcdoc)) mask.push("tag");
...
function handleDpchEngSub(_scrJref, dom, dpch) {
	...
	} else if (_scrJref == scrJrefConfig) {
		doc.getElementById("Config").contentWindow.handleDpchEng(dom, dpch);
	} else if (_scrJref == scrJrefMonitor) {
		doc.getElementById("Monitor").contentWindow.handleDpchEng(dom, dpch);
	} else if (_scrJref == scrJrefDebug) {
		doc.getElementById("Debug").contentWindow.handleDpchEng(dom, dpch);
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWzskVtrData") {
				mergeDpchEngData(dom);

				// establish sub-panel scrJref's as global variables, and start sub-panel load sequence
				scrJrefConfig = retrieveSi(srcdoc, "StatShrWzskVtr", "scrJrefConfig");
				scrJrefMonitor = retrieveSi(srcdoc, "StatShrWzskVtr", "scrJrefMonitor");
				scrJrefDebug = retrieveSi(srcdoc, "StatShrWzskVtr", "scrJrefDebug");
				scrJrefHeadbar = retrieveSi(srcdoc, "StatShrWzskVtr", "scrJrefHeadbar");
		...
		if (blk) {
			...
			if (_scrJref != "") {
				if (_scrJref == scrJref) {
					...
					} else if (blk.nodeName == "DpchEngWzskVtrData") {
						mergeDpchEngData(dom);
						refresh();
					};
...
function restartNotify() {
	var srefIxWzskVReqitmode = retrieveSi(srcdoc, "StatAppWzskVtr", "srefIxWzskVReqitmode");

	if (srefIxWzskVReqitmode != "notify") {
		if (doc.timeout) window.clearTimeout(doc.timeout);

		srefIxWzskVReqitmode = "notify";
		setSi(srcdoc, "StatAppWzskVtr", "srefIxWzskVReqitmode", srefIxWzskVReqitmode);
...
function iterateReqit(ev) {
	...
	var srefIxWzskVReqitmode = retrieveSi(srcdoc, "StatAppWzskVtr", "srefIxWzskVReqitmode");
	var latency = parseInt(retrieveSi(srcdoc, "StatAppWzskVtr", "latency"));
