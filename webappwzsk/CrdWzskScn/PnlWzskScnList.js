/**
  * \file PnlWzskScnList.js
  * web client functionality for panel PnlWzskScnList
...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlWzskScnList_aside.html";
	doc.getElementById("List_cont").src = "./PnlWzskScnList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlWzskScnList_bside.html";
	doc.getElementById("List_cont").src = "./PnlWzskScnList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWzskScnList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWzskScnList", "Trs"));
...
function init() {
	var srefIxWzskVExpstate = retrieveSi(srcdoc, "StatShrWzskScnList", "srefIxWzskVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWzskScnList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWzskScnList", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWzskScnList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzskScnList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzskScnList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWzskScnList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWzskScnList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWzskScnList", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWzskScnList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWzskScnList", "ncol"));

		var TcoTitWidth = parseInt(retrieveSi(srcdoc, "StgIacWzskScnList", "TcoTitWidth"));
		var TcoStaWidth = parseInt(retrieveSi(srcdoc, "StgIacWzskScnList", "TcoStaWidth"));
		var TcoStoWidth = parseInt(retrieveSi(srcdoc, "StgIacWzskScnList", "TcoStoWidth"));
		var TcoLatWidth = parseInt(retrieveSi(srcdoc, "StgIacWzskScnList", "TcoLatWidth"));
		var TcoLngWidth = parseInt(retrieveSi(srcdoc, "StgIacWzskScnList", "TcoLngWidth"));
		var TcoElvWidth = parseInt(retrieveSi(srcdoc, "StgIacWzskScnList", "TcoElvWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacWzskScnList", "numFTos"));
		...
		for (i = firstcol, ix = 0; i <= ncol; i++) {
			...
			if (i == 1) {
				...
				doc.cols.push("TcoLat"); doc.widths.push(TcoLatWidth); doc.fs.push("lat"); doc.tos.push(0); wtot += TcoLatWidth;
			} else if (i == 5) {
				doc.cols.push("TcoLng"); doc.widths.push(TcoLngWidth); doc.fs.push("lng"); doc.tos.push(0); wtot += TcoLngWidth;
			} else if (i == 6) {
				doc.cols.push("TcoElv"); doc.widths.push(TcoElvWidth); doc.fs.push("elv"); doc.tos.push(0); wtot += TcoElvWidth;
			} else {
				ix--;
			};
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagWzskScnList", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWzskScnList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWzskQScnList
		rownode = getRownode(srcdoc, "ListWzskQScnList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWzskScnList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWzskScnList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWzskScnList", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WzskScnList", "ListWzskQScnList", retrieveTi(srcdoc, "TagWzskScnList", "TxtRecord1"), retrieveTi(srcdoc, "TagWzskScnList", "TxtRecord2"),
				["Tit","Sta","Sto","Lat","Lng","Elv"],
				["tit","sta","sto","lat","lng","elv"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWzskScnList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzskScnList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWzskScnList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWzskScnList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWzskScnList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWzskScnListData", scrJref, "StgIacQryWzskScnList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWzskScnList", "firstcol"));

	setSi(srcdoc, "StatAppQryWzskScnList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWzskScnList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWzskScnListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacWzskScnList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacWzskScnList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppWzskScnListData", scrJref, "ContIacWzskScnList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWzskScnListData", "ContIacWzskScnList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngWzskScnListData", "ContInfWzskScnList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWzskScnListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWzskScnListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngWzskScnListData", "StatShrWzskScnList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWzskScnListData", "StgIacWzskScnList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWzskScnListData", "TagWzskScnList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWzskScnListData", "ListWzskQScnList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWzskScnListData", "StatAppQryWzskScnList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWzskScnListData", "StatShrQryWzskScnList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWzskScnListData", "StgIacQryWzskScnList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngWzskScnListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWzskScnListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWzskScnListData") {
				mask = mergeDpchEngData(dom);
