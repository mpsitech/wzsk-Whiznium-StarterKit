/**
  * \file PnlWzskPrsADetail.js
  * web client functionality for panel PnlWzskPrsADetail
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWzskPrsADetail", "srefIxWzskVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWzskPrsADetail", "srefIxWzskVExpstate", "mind");
	...
	// change content
	doc.getElementById("ADetail_side").src = "./PnlWzskPrsADetail_aside.html";
	doc.getElementById("ADetail_cont").src = "./PnlWzskPrsADetail_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWzskPrsADetail", "srefIxWzskVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWzskPrsADetail", "srefIxWzskVExpstate", "regd");
	...
	// change content
	doc.getElementById("ADetail_side").src = "./PnlWzskPrsADetail_bside.html";
	doc.getElementById("ADetail_cont").src = "./PnlWzskPrsADetail_b.html";
};
...
function initBD(bNotD) {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWzskPrsADetail", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWzskPrsADetail", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWzskVExpstate = retrieveSi(srcdoc, "StatAppWzskPrsADetail", "srefIxWzskVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWzskPrsADetail", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWzskPrsADetail", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWzskPrsADetail", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzskPrsADetail", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzskPrsADetail", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWzskPrsADetail", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWzskPrsADetail", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWzskPrsADetail", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWzskPrsADetail", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWzskPrsADetail", "ncol"));

		var TcoTypWidth = parseInt(retrieveSi(srcdoc, "StgIacWzskPrsADetail", "TcoTypWidth"));
		var TcoValWidth = parseInt(retrieveSi(srcdoc, "StgIacWzskPrsADetail", "TcoValWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWzskPrsADetail", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWzskPrsADetail", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWzskQPrsADetail
		rownode = getRownode(srcdoc, "ListWzskQPrsADetail", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWzskPrsADetail", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWzskPrsADetail", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWzskPrsADetail", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WzskPrsADetail", "ListWzskQPrsADetail", retrieveTi(srcdoc, "TagWzskPrsADetail", "TxtRecord1"), retrieveTi(srcdoc, "TagWzskPrsADetail", "TxtRecord2"),
				["Typ","","Val"],
				["typ","typ2","val"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWzskPrsADetail", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzskPrsADetail", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWzskPrsADetail", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWzskPrsADetail", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWzskPrsADetail", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWzskPrsADetailData", scrJref, "StgIacQryWzskPrsADetail");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWzskPrsADetail", "srefIxWzskVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWzskPrsADetail", "firstcol"));

	setSi(srcdoc, "StatAppQryWzskPrsADetail", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWzskPrsADetail", "srefIxWzskVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWzskPrsADetail", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWzskPrsADetailDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWzskPrsADetailData", "ContInfWzskPrsADetail", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWzskPrsADetailData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWzskPrsADetailData", "StatAppWzskPrsADetail", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWzskPrsADetailData", "StatShrWzskPrsADetail", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWzskPrsADetailData", "StgIacWzskPrsADetail", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWzskPrsADetailData", "TagWzskPrsADetail", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWzskPrsADetailData", "ListWzskQPrsADetail", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWzskPrsADetailData", "StatAppQryWzskPrsADetail", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWzskPrsADetailData", "StatShrQryWzskPrsADetail", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWzskPrsADetailData", "StgIacQryWzskPrsADetail", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWzskPrsADetailData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWzskPrsADetailData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWzskPrsADetailData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
