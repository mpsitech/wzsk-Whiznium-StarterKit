/**
  * \file PnlWzskPrsList.js
  * web client functionality for panel PnlWzskPrsList
...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlWzskPrsList_aside.html";
	doc.getElementById("List_cont").src = "./PnlWzskPrsList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlWzskPrsList_bside.html";
	doc.getElementById("List_cont").src = "./PnlWzskPrsList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWzskPrsList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWzskPrsList", "Trs"));
...
function init() {
	var srefIxWzskVExpstate = retrieveSi(srcdoc, "StatShrWzskPrsList", "srefIxWzskVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWzskPrsList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWzskPrsList", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWzskPrsList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzskPrsList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzskPrsList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWzskPrsList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWzskPrsList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWzskPrsList", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWzskPrsList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWzskPrsList", "ncol"));

		var TcoGrpWidth = parseInt(retrieveSi(srcdoc, "StgIacWzskPrsList", "TcoGrpWidth"));
		var TcoOwnWidth = parseInt(retrieveSi(srcdoc, "StgIacWzskPrsList", "TcoOwnWidth"));
		var TcoTitWidth = parseInt(retrieveSi(srcdoc, "StgIacWzskPrsList", "TcoTitWidth"));
		var TcoFnmWidth = parseInt(retrieveSi(srcdoc, "StgIacWzskPrsList", "TcoFnmWidth"));
		var TcoLnmWidth = parseInt(retrieveSi(srcdoc, "StgIacWzskPrsList", "TcoLnmWidth"));
		var TcoSexWidth = parseInt(retrieveSi(srcdoc, "StgIacWzskPrsList", "TcoSexWidth"));
		var TcoTelWidth = parseInt(retrieveSi(srcdoc, "StgIacWzskPrsList", "TcoTelWidth"));
		var TcoEmlWidth = parseInt(retrieveSi(srcdoc, "StgIacWzskPrsList", "TcoEmlWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacWzskPrsList", "numFTos"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagWzskPrsList", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWzskPrsList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWzskQPrsList
		rownode = getRownode(srcdoc, "ListWzskQPrsList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWzskPrsList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWzskPrsList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWzskPrsList", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WzskPrsList", "ListWzskQPrsList", retrieveTi(srcdoc, "TagWzskPrsList", "TxtRecord1"), retrieveTi(srcdoc, "TagWzskPrsList", "TxtRecord2"),
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWzskPrsList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzskPrsList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWzskPrsList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWzskPrsList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWzskPrsList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWzskPrsListData", scrJref, "StgIacQryWzskPrsList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWzskPrsList", "firstcol"));

	setSi(srcdoc, "StatAppQryWzskPrsList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWzskPrsList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWzskPrsListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacWzskPrsList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacWzskPrsList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppWzskPrsListData", scrJref, "ContIacWzskPrsList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWzskPrsListData", "ContIacWzskPrsList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngWzskPrsListData", "ContInfWzskPrsList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWzskPrsListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWzskPrsListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngWzskPrsListData", "StatShrWzskPrsList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWzskPrsListData", "StgIacWzskPrsList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWzskPrsListData", "TagWzskPrsList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWzskPrsListData", "ListWzskQPrsList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWzskPrsListData", "StatAppQryWzskPrsList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWzskPrsListData", "StatShrQryWzskPrsList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWzskPrsListData", "StgIacQryWzskPrsList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngWzskPrsListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWzskPrsListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWzskPrsListData") {
				mask = mergeDpchEngData(dom);
