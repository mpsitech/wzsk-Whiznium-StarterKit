/**
	* \file WzskQOgrSup1NObjgroup.swift
	* Swift API code for table TblWzskQOgrSup1NObjgroup
...
public class WzskQOgrSup1NObjgroup {
	...
	public func readXML(
		...
		if addbasetag {basexpath = Xmlio.checkUclcXPaths(doc, _basexpath, "WzskQOgrSup1NObjgroup")}
		valid = basexpath != nil
...
public class ListWzskQOgrSup1NObjgroup {
	...
	public var nodes: [WzskQOgrSup1NObjgroup] = []
	...
	public func readXML(
		...
		var rec: WzskQOgrSup1NObjgroup

		var s: String

		if addbasetag {basexpath = Xmlio.checkUclcXPaths(doc, _basexpath, "ListWzskQOgrSup1NObjgroup")}
		...
		if basexpath != nil {
			if (Xmlio.checkXPath(doc, basexpath!)) {
				...
				for i in 0 ... ics.count - 1 {
					rec = WzskQOgrSup1NObjgroup()

					if shorttags[i] {s = basexpath! + "/row[@jnum='" + String(ics[i]) + "']"}
					else {s = basexpath! + "/WzskQOgrSup1NObjgroup[@jnum='" + String(ics[i]) + "']"}
