...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlWzskShtList.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppWzskShtListData": {
						"scrJref": this.scrJref,
						"StgIacQryWzskShtList": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacWzskShtList"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppWzskShtListData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppWzskShtListData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContIacWzskShtList) this.contiac = dpcheng.ContIacWzskShtList;
				if (dpcheng.ContInfWzskShtList) this.continf = dpcheng.ContInfWzskShtList;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.FeedFTos) this.feedFTos = dpcheng.FeedFTos;
				if (dpcheng.StatShrWzskShtList) this.statshr = dpcheng.StatShrWzskShtList;
				if (dpcheng.StgIacWzskShtList) this.stgiac = dpcheng.StgIacWzskShtList;
				if (dpcheng.TagWzskShtList) {
					Wzsk.unescapeBlock(dpcheng.TagWzskShtList);
					this.tag = dpcheng.TagWzskShtList;
				}
				if (dpcheng.ListWzskQShtList) this.rst = dpcheng.ListWzskQShtList;
				if (dpcheng.StatAppQryWzskShtList) this.statappqry = dpcheng.StatAppQryWzskShtList;
				if (dpcheng.StatShrQryWzskShtList) this.statshrqry = dpcheng.StatShrQryWzskShtList;
				if (dpcheng.StgIacQryWzskShtList) this.stgiacqry = dpcheng.StgIacQryWzskShtList;
				if (dpcheng.ContInfWzskShtList) {
				...
				*/
				...
				if (dpcheng.TagWzskShtList) {
					...
					/*
					*/
					tcos.push({value: "jnum", text: "jnum"}, {value: "TcoSes", text: this.tag.TcoSes}, {value: "TcoObj", text: this.tag.TcoObj}, {value: "TcoSta", text: this.tag.TcoSta});
					/*
					*/
				...
				if (dpcheng.ListWzskQShtList) {
					...
					for (var i = 0; i < this.rst.length; i++) {
						...
						/*
						*/
						rows.push({jnum: rec["jnum"], TcoSes: rec["ses"], TcoObj: rec["obj"], TcoSta: rec["sta"]})
						/*
						*/
			...
			handleDpchAppDataDoReply: function(srefIxWzskVDpch, dpcheng) {
				if (srefIxWzskVDpch == "DpchEngWzskShtListData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxWzskVDpch == "DpchEngWzskShtListData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
