...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlWzskOgrList.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppWzskOgrListData": {
						"scrJref": this.scrJref,
						"StgIacQryWzskOgrList": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacWzskOgrList"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppWzskOgrListData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppWzskOgrListData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContIacWzskOgrList) this.contiac = dpcheng.ContIacWzskOgrList;
				if (dpcheng.ContInfWzskOgrList) this.continf = dpcheng.ContInfWzskOgrList;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.FeedFTos) this.feedFTos = dpcheng.FeedFTos;
				if (dpcheng.StatShrWzskOgrList) this.statshr = dpcheng.StatShrWzskOgrList;
				if (dpcheng.StgIacWzskOgrList) this.stgiac = dpcheng.StgIacWzskOgrList;
				if (dpcheng.TagWzskOgrList) {
					Wzsk.unescapeBlock(dpcheng.TagWzskOgrList);
					this.tag = dpcheng.TagWzskOgrList;
				}
				if (dpcheng.ListWzskQOgrList) this.rst = dpcheng.ListWzskQOgrList;
				if (dpcheng.StatAppQryWzskOgrList) this.statappqry = dpcheng.StatAppQryWzskOgrList;
				if (dpcheng.StatShrQryWzskOgrList) this.statshrqry = dpcheng.StatShrQryWzskOgrList;
				if (dpcheng.StgIacQryWzskOgrList) this.stgiacqry = dpcheng.StgIacQryWzskOgrList;
				if (dpcheng.ContInfWzskOgrList) {
				...
				*/
				...
				if (dpcheng.TagWzskOgrList) {
					...
					/*
					*/
					tcos.push({value: "jnum", text: "jnum"}, {value: "TcoSrf", text: this.tag.TcoSrf}, {value: "TcoTit", text: this.tag.TcoTit}, {value: "TcoSup", text: this.tag.TcoSup});
					/*
					*/
				...
				if (dpcheng.ListWzskQOgrList) {
					...
					for (var i = 0; i < this.rst.length; i++) {
						...
						/*
						*/
						rows.push({jnum: rec["jnum"], TcoSrf: rec["srf"], TcoTit: rec["tit"], TcoSup: rec["sup"]})
						/*
						*/
			...
			handleDpchAppDataDoReply: function(srefIxWzskVDpch, dpcheng) {
				if (srefIxWzskVDpch == "DpchEngWzskOgrListData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxWzskVDpch == "DpchEngWzskOgrListData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
