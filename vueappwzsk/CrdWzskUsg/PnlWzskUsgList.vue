...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlWzskUsgList.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppWzskUsgListData": {
						"scrJref": this.scrJref,
						"StgIacQryWzskUsgList": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacWzskUsgList"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppWzskUsgListData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppWzskUsgListData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContIacWzskUsgList) this.contiac = dpcheng.ContIacWzskUsgList;
				if (dpcheng.ContInfWzskUsgList) this.continf = dpcheng.ContInfWzskUsgList;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.FeedFTos) this.feedFTos = dpcheng.FeedFTos;
				if (dpcheng.StatShrWzskUsgList) this.statshr = dpcheng.StatShrWzskUsgList;
				if (dpcheng.StgIacWzskUsgList) this.stgiac = dpcheng.StgIacWzskUsgList;
				if (dpcheng.TagWzskUsgList) {
					Wzsk.unescapeBlock(dpcheng.TagWzskUsgList);
					this.tag = dpcheng.TagWzskUsgList;
				}
				if (dpcheng.ListWzskQUsgList) this.rst = dpcheng.ListWzskQUsgList;
				if (dpcheng.StatAppQryWzskUsgList) this.statappqry = dpcheng.StatAppQryWzskUsgList;
				if (dpcheng.StatShrQryWzskUsgList) this.statshrqry = dpcheng.StatShrQryWzskUsgList;
				if (dpcheng.StgIacQryWzskUsgList) this.stgiacqry = dpcheng.StgIacQryWzskUsgList;
				if (dpcheng.ContInfWzskUsgList) {
			...
			handleDpchAppDataDoReply: function(srefIxWzskVDpch, dpcheng) {
				if (srefIxWzskVDpch == "DpchEngWzskUsgListData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxWzskVDpch == "DpchEngWzskUsgListData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
