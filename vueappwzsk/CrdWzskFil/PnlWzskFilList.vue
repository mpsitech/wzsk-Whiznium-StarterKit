...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlWzskFilList.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppWzskFilListData": {
						"scrJref": this.scrJref,
						"StgIacQryWzskFilList": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacWzskFilList"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppWzskFilListData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppWzskFilListData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContIacWzskFilList) this.contiac = dpcheng.ContIacWzskFilList;
				if (dpcheng.ContInfWzskFilList) this.continf = dpcheng.ContInfWzskFilList;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.FeedFTos) this.feedFTos = dpcheng.FeedFTos;
				if (dpcheng.StatShrWzskFilList) this.statshr = dpcheng.StatShrWzskFilList;
				if (dpcheng.StgIacWzskFilList) this.stgiac = dpcheng.StgIacWzskFilList;
				if (dpcheng.TagWzskFilList) {
					Wzsk.unescapeBlock(dpcheng.TagWzskFilList);
					this.tag = dpcheng.TagWzskFilList;
				}
				if (dpcheng.ListWzskQFilList) this.rst = dpcheng.ListWzskQFilList;
				if (dpcheng.StatAppQryWzskFilList) this.statappqry = dpcheng.StatAppQryWzskFilList;
				if (dpcheng.StatShrQryWzskFilList) this.statshrqry = dpcheng.StatShrQryWzskFilList;
				if (dpcheng.StgIacQryWzskFilList) this.stgiacqry = dpcheng.StgIacQryWzskFilList;
				if (dpcheng.ContInfWzskFilList) {
			...
			handleDpchAppDataDoReply: function(srefIxWzskVDpch, dpcheng) {
				if (srefIxWzskVDpch == "DpchEngWzskFilListData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxWzskVDpch == "DpchEngWzskFilListData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
